...
4. **Wrap first, reimplement second** — integrate FreeSurfer SynthSeg, FastSurfer, FSL, ANTs via **thin adapters** (prefer `deepsynaps_mri/adapters/` as code lands; today some CLIs are still called from `io.py` / `structural.py` / `qc.py`) and subprocess/container calls; do not rewrite recon-all or ANTs in one pass.
