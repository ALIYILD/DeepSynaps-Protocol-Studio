...
def test_search_coordinate_query_preserves_provenance(client, auth_headers):
    """Live runtime (PR-3): enabled sources surface one of
    ok/degraded/timeout/error — never the PR-1 stub vocabulary.

    Provenance still records the queried-sources list + disclaimer.
    """
    # Force every adapter to fail-fast (no real network I/O in tests).
    from app.routers import neuroimaging_knowledge_router as nr
    ...
    assert body["provenance"]["total_results"] == len(body["results"])
