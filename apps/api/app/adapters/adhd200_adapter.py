...
BASE_URL = "http://fcon_1000.projects.nitrc.org/indi/adhd200"
...
class ADHD200Adapter:
    ...
    async def _rate_limited_get(self, url: str, params: Optional[Dict] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            raise ADHD200APIError(f"HTTP {e.response.status_code}: {e.response.text}") from e
        except httpx.RequestError as e:
            raise ADHD200APIError(f"Request failed: {e}") from e

    async def search(self, query: str, limit: int = 10) -> List[ADHD200Dataset]:
        """Search datasets."""
    ...
    async def get_dataset(self, dataset_id: str) -> Optional[ADHD200Dataset]:
        """Get dataset by ID."""
    ...
    async def get_datasets(self, limit: int = 10) -> List[ADHD200Dataset]:
        """List all datasets."""
        url = f"{BASE_URL}/datasets"
    ...
    async def search_by_modality(self, modality: str, limit: int = 10) -> List[ADHD200Dataset]:
        """Search by modality."""
        url = f"{BASE_URL}/datasets"
    ...
    async def search_by_task(self, task: str, limit: int = 10) -> List[ADHD200Dataset]:
        """Search by task."""
        url = f"{BASE_URL}/datasets"
    ...
    async def get_recent(self, limit: int = 10) -> List[ADHD200Dataset]:
        """Get recent datasets."""
        url = f"{BASE_URL}/datasets"
    ...
    async def get_items(self, limit: int = 10) -> List[ADHD200Item]:
        """Get items."""
    ...
    async def search_items(self, query: str, limit: int = 10) -> List[ADHD200Item]:
        """Search items."""
        url = f"{BASE_URL}/items"
        params: Dict[str, Any] = {"q": query, "limit": min(limit, 1000)}
        data = await self._rate_limited_get(url, params)
    ...
    async def get_item(self, item_id: str) -> Optional[ADHD200Item]:
        """Get item by ID."""
        url = f"{BASE_URL}/items/{item_id}"
        data = await self._rate_limited_get(url)
        return self._parse_item(data) if "id" in data else None

    def _parse_dataset(self, data: Dict[str, Any]) -> ADHD200Dataset:
    ...
    def _parse_item(self, data: Dict[str, Any]) -> ADHD200Item:
        return ADHD200Item(
            id=str(data.get("id", "")),
    ...
    async def __aenter__(self) -> "ADHD200Adapter":
        return self
...
class ADHD200APIError(Exception):
    pass


async def _test_adhd200() -> None:
    adapter = ADHD200Adapter()
    ...
    await adapter.close()
    print("[TEST] All ADHD200 tests completed.")


if __name__ == "__main__":
    asyncio.run(_test_adhd200())
