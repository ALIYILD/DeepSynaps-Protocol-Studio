...
BASE_URL = "https://www.oasis-brains.org"
...
class OASISAdapter:
    ...
    async def _rate_limited_get(self, url: str, params: Optional[Dict] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            raise OASISAPIError(f"HTTP {e.response.status_code}: {e.response.text}") from e
        except httpx.RequestError as e:
            raise OASISAPIError(f"Request failed: {e}") from e

    async def search(self, query: str, limit: int = 10) -> List[OASISDataset]:
        """Search datasets."""
    ...
    async def get_dataset(self, dataset_id: str) -> Optional[OASISDataset]:
        """Get dataset by ID."""
    ...
    async def get_datasets(self, limit: int = 10) -> List[OASISDataset]:
        """List all datasets."""
        url = f"{BASE_URL}/datasets"
    ...
    async def search_by_modality(self, modality: str, limit: int = 10) -> List[OASISDataset]:
        """Search by modality."""
        url = f"{BASE_URL}/datasets"
    ...
    async def search_by_task(self, task: str, limit: int = 10) -> List[OASISDataset]:
        """Search by task."""
        url = f"{BASE_URL}/datasets"
    ...
    async def get_recent(self, limit: int = 10) -> List[OASISDataset]:
        """Get recent datasets."""
        url = f"{BASE_URL}/datasets"
    ...
    async def get_items(self, limit: int = 10) -> List[OASISItem]:
        """Get items."""
    ...
    async def search_items(self, query: str, limit: int = 10) -> List[OASISItem]:
        """Search items."""
        url = f"{BASE_URL}/items"
        params: Dict[str, Any] = {"q": query, "limit": min(limit, 1000)}
        data = await self._rate_limited_get(url, params)
    ...
    async def get_item(self, item_id: str) -> Optional[OASISItem]:
        """Get item by ID."""
        url = f"{BASE_URL}/items/{item_id}"
        data = await self._rate_limited_get(url)
        return self._parse_item(data) if "id" in data else None

    def _parse_dataset(self, data: Dict[str, Any]) -> OASISDataset:
        return OASISDataset(
            id=str(data.get("id", "")),
    ...
    def _parse_item(self, data: Dict[str, Any]) -> OASISItem:
        return OASISItem(
            id=str(data.get("id", "")),
    ...
    async def __aenter__(self) -> "OASISAdapter":
        return self
...
async def _test_oasis() -> None:
    ...
    await adapter.close()
    print("[TEST] All OASIS tests completed.")


if __name__ == "__main__":
    asyncio.run(_test_oasis())
