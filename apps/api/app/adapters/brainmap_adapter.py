...
BrainMap neuroimaging meta-analysis database.
"""
...
BASE_URL = "http://www.brainmap.org"
...
class BrainMapAdapter:
    """Async adapter for BrainMap.

    Example:
        adapter = BrainMapAdapter()
    ...
    async def _rate_limited_get(self, url: str, params: Optional[Dict] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            raise BrainMapAPIError(f"HTTP {e.response.status_code}: {e.response.text}") from e
        except httpx.RequestError as e:
            raise BrainMapAPIError(f"Request failed: {e}") from e

    async def search(self, query: str, limit: int = 10) -> List[BrainMapDataset]:
        """Search datasets."""
    ...
    async def get_dataset(self, dataset_id: str) -> Optional[BrainMapDataset]:
        """Get dataset by ID."""
    ...
    async def get_datasets(self, limit: int = 10) -> List[BrainMapDataset]:
        """List all datasets."""
        url = f"{BASE_URL}/datasets"
    ...
    async def search_by_modality(self, modality: str, limit: int = 10) -> List[BrainMapDataset]:
        """Search by modality."""
        url = f"{BASE_URL}/datasets"
    ...
    async def search_by_task(self, task: str, limit: int = 10) -> List[BrainMapDataset]:
        """Search by task."""
        url = f"{BASE_URL}/datasets"
    ...
    async def get_recent(self, limit: int = 10) -> List[BrainMapDataset]:
        """Get recent datasets."""
        url = f"{BASE_URL}/datasets"
    ...
    async def get_items(self, limit: int = 10) -> List[BrainMapItem]:
        """Get items."""
    ...
    async def search_items(self, query: str, limit: int = 10) -> List[BrainMapItem]:
        """Search items."""
        url = f"{BASE_URL}/items"
        params: Dict[str, Any] = {"q": query, "limit": min(limit, 1000)}
        data = await self._rate_limited_get(url, params)
    ...
    async def get_item(self, item_id: str) -> Optional[BrainMapItem]:
        """Get item by ID."""
        url = f"{BASE_URL}/items/{item_id}"
        data = await self._rate_limited_get(url)
        return self._parse_item(data) if "id" in data else None

    def _parse_dataset(self, data: Dict[str, Any]) -> BrainMapDataset:
        return BrainMapDataset(
            id=str(data.get("id", "")),
    ...
    def _parse_item(self, data: Dict[str, Any]) -> BrainMapItem:
        return BrainMapItem(
            id=str(data.get("id", "")),
    ...
    async def __aenter__(self) -> "BrainMapAdapter":
        return self
...
class BrainMapAPIError(Exception):
    pass


async def _test_brainmap() -> None:
    adapter = BrainMapAdapter()
    ...
    await adapter.close()
    print("[TEST] All BrainMap tests completed.")


if __name__ == "__main__":
    asyncio.run(_test_brainmap())
