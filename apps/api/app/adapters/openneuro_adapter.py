...
"""
OpenNeuro Adapter
https://openneuro.org/crn/

OpenNeuro neuroimaging dataset repository.
"""
...
class OpenNeuroAdapter:
    ...
    async def _rate_limited_get(self, url: str, params: Optional[Dict] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            raise OpenNeuroAPIError(f"HTTP {e.response.status_code}: {e.response.text}") from e
        except httpx.RequestError as e:
            raise OpenNeuroAPIError(f"Request failed: {e}") from e

    async def search(self, query: str, limit: int = 10) -> List[OpenNeuroDataset]:
        """Search datasets."""
    ...
    async def get_dataset(self, dataset_id: str) -> Optional[OpenNeuroDataset]:
        """Get dataset by ID."""
    ...
    async def get_datasets(self, limit: int = 10) -> List[OpenNeuroDataset]:
        """List all datasets."""
        url = f"{BASE_URL}/datasets"
    ...
    async def search_by_modality(self, modality: str, limit: int = 10) -> List[OpenNeuroDataset]:
        """Search by modality."""
        url = f"{BASE_URL}/datasets"
    ...
    async def search_by_task(self, task: str, limit: int = 10) -> List[OpenNeuroDataset]:
        """Search by task."""
        url = f"{BASE_URL}/datasets"
    ...
    async def get_recent(self, limit: int = 10) -> List[OpenNeuroDataset]:
        """Get recent datasets."""
        url = f"{BASE_URL}/datasets"
    ...
    async def get_items(self, limit: int = 10) -> List[OpenNeuroItem]:
        """Get items."""
    ...
    async def search_items(self, query: str, limit: int = 10) -> List[OpenNeuroItem]:
        """Search items."""
        url = f"{BASE_URL}/items"
        params: Dict[str, Any] = {"q": query, "limit": min(limit, 1000)}
        data = await self._rate_limited_get(url, params)
    ...
    async def get_item(self, item_id: str) -> Optional[OpenNeuroItem]:
        """Get item by ID."""
        url = f"{BASE_URL}/items/{item_id}"
        data = await self._rate_limited_get(url)
        return self._parse_item(data) if "id" in data else None

    def _parse_dataset(self, data: Dict[str, Any]) -> OpenNeuroDataset:
    ...
    def _parse_item(self, data: Dict[str, Any]) -> OpenNeuroItem:
        return OpenNeuroItem(
            id=str(data.get("id", "")),
    ...
    async def __aenter__(self) -> "OpenNeuroAdapter":
        return self
...
class OpenNeuroAPIError(Exception):
    pass


async def _test_openneuro() -> None:
    adapter = OpenNeuroAdapter()
    ...
    await adapter.close()
    print("[TEST] All OpenNeuro tests completed.")


if __name__ == "__main__":
    asyncio.run(_test_openneuro())
