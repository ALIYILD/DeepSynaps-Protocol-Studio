...
EBRAINS digital neuroscience research infrastructure.
"""
...
class EBRAINSAdapter:
    """Async adapter for EBRAINS.

    Example:
        adapter = EBRAINSAdapter()
    ...
    async def _rate_limited_get(self, url: str, params: Optional[Dict] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            raise EBRAINSAPIError(f"HTTP {e.response.status_code}: {e.response.text}") from e
        except httpx.RequestError as e:
            raise EBRAINSAPIError(f"Request failed: {e}") from e

    async def search(self, query: str, limit: int = 10) -> List[EBRAINSDataset]:
        """Search datasets."""
    ...
    async def get_dataset(self, dataset_id: str) -> Optional[EBRAINSDataset]:
        """Get dataset by ID."""
    ...
    async def get_datasets(self, limit: int = 10) -> List[EBRAINSDataset]:
        """List all datasets."""
        url = f"{BASE_URL}/datasets"
    ...
    async def search_by_modality(self, modality: str, limit: int = 10) -> List[EBRAINSDataset]:
        """Search by modality."""
        url = f"{BASE_URL}/datasets"
    ...
    async def search_by_task(self, task: str, limit: int = 10) -> List[EBRAINSDataset]:
        """Search by task."""
        url = f"{BASE_URL}/datasets"
    ...
    async def get_recent(self, limit: int = 10) -> List[EBRAINSDataset]:
        """Get recent datasets."""
        url = f"{BASE_URL}/datasets"
    ...
    async def get_items(self, limit: int = 10) -> List[EBRAINSItem]:
        """Get items."""
    ...
    async def search_items(self, query: str, limit: int = 10) -> List[EBRAINSItem]:
        """Search items."""
        url = f"{BASE_URL}/items"
        params: Dict[str, Any] = {"q": query, "limit": min(limit, 1000)}
        data = await self._rate_limited_get(url, params)
    ...
    async def get_item(self, item_id: str) -> Optional[EBRAINSItem]:
        """Get item by ID."""
        url = f"{BASE_URL}/items/{item_id}"
        data = await self._rate_limited_get(url)
        return self._parse_item(data) if "id" in data else None

    def _parse_dataset(self, data: Dict[str, Any]) -> EBRAINSDataset:
    ...
    def _parse_item(self, data: Dict[str, Any]) -> EBRAINSItem:
        return EBRAINSItem(
            id=str(data.get("id", "")),
    ...
    async def __aenter__(self) -> "EBRAINSAdapter":
        return self
...
class EBRAINSAPIError(Exception):
    pass


async def _test_ebrains() -> None:
    adapter = EBRAINSAdapter()
    ...
    await adapter.close()
    print("[TEST] All EBRAINS tests completed.")


if __name__ == "__main__":
    asyncio.run(_test_ebrains())
