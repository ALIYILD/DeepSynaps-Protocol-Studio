...
SimNIBS brain stimulation simulation platform.
"""
...
BASE_URL = "https://simnibs.github.io"
...
class SimNIBSAdapter:
    ...
    async def _rate_limited_get(self, url: str, params: Optional[Dict] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            raise SimNIBSAPIError(f"HTTP {e.response.status_code}: {e.response.text}") from e
        except httpx.RequestError as e:
            raise SimNIBSAPIError(f"Request failed: {e}") from e

    async def search(self, query: str, limit: int = 10) -> List[SimNIBSDataset]:
        """Search datasets."""
    ...
    async def get_dataset(self, dataset_id: str) -> Optional[SimNIBSDataset]:
        """Get dataset by ID."""
    ...
    async def get_datasets(self, limit: int = 10) -> List[SimNIBSDataset]:
        """List all datasets."""
        url = f"{BASE_URL}/datasets"
    ...
    async def search_by_modality(self, modality: str, limit: int = 10) -> List[SimNIBSDataset]:
        """Search by modality."""
        url = f"{BASE_URL}/datasets"
    ...
    async def search_by_task(self, task: str, limit: int = 10) -> List[SimNIBSDataset]:
        """Search by task."""
        url = f"{BASE_URL}/datasets"
    ...
    async def get_recent(self, limit: int = 10) -> List[SimNIBSDataset]:
        """Get recent datasets."""
        url = f"{BASE_URL}/datasets"
    ...
    async def get_items(self, limit: int = 10) -> List[SimNIBSItem]:
        """Get items."""
    ...
    async def search_items(self, query: str, limit: int = 10) -> List[SimNIBSItem]:
        """Search items."""
        url = f"{BASE_URL}/items"
        params: Dict[str, Any] = {"q": query, "limit": min(limit, 1000)}
        data = await self._rate_limited_get(url, params)
    ...
    async def get_item(self, item_id: str) -> Optional[SimNIBSItem]:
        """Get item by ID."""
        url = f"{BASE_URL}/items/{item_id}"
        data = await self._rate_limited_get(url)
        return self._parse_item(data) if "id" in data else None

    def _parse_dataset(self, data: Dict[str, Any]) -> SimNIBSDataset:
        return SimNIBSDataset(
            id=str(data.get("id", "")),
    ...
    def _parse_item(self, data: Dict[str, Any]) -> SimNIBSItem:
        return SimNIBSItem(
            id=str(data.get("id", "")),
    ...
    async def __aenter__(self) -> "SimNIBSAdapter":
        return self
...
class SimNIBSAPIError(Exception):
    pass


async def _test_simnibs() -> None:
    adapter = SimNIBSAdapter()
    ...
    await adapter.close()
    print("[TEST] All SimNIBS tests completed.")


if __name__ == "__main__":
    asyncio.run(_test_simnibs())
