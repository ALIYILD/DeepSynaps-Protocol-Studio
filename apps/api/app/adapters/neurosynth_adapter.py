...
Neurosynth meta-analysis platform.
"""
...
BASE_URL = "https://github.com/neurosynth/neurosynth"
...
class NeurosynthAdapter:
    """Async adapter for Neurosynth.

    Example:
        adapter = NeurosynthAdapter()
    ...
    async def _rate_limited_get(self, url: str, params: Optional[Dict] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            raise NeurosynthAPIError(f"HTTP {e.response.status_code}: {e.response.text}") from e
        except httpx.RequestError as e:
            raise NeurosynthAPIError(f"Request failed: {e}") from e

    async def search(self, query: str, limit: int = 10) -> List[NeurosynthDataset]:
        """Search datasets."""
    ...
    async def get_dataset(self, dataset_id: str) -> Optional[NeurosynthDataset]:
        """Get dataset by ID."""
    ...
    async def get_datasets(self, limit: int = 10) -> List[NeurosynthDataset]:
        """List all datasets."""
        url = f"{BASE_URL}/datasets"
    ...
    async def search_by_modality(self, modality: str, limit: int = 10) -> List[NeurosynthDataset]:
        """Search by modality."""
        url = f"{BASE_URL}/datasets"
    ...
    async def search_by_task(self, task: str, limit: int = 10) -> List[NeurosynthDataset]:
        """Search by task."""
        url = f"{BASE_URL}/datasets"
    ...
    async def get_recent(self, limit: int = 10) -> List[NeurosynthDataset]:
        """Get recent datasets."""
        url = f"{BASE_URL}/datasets"
    ...
    async def get_items(self, limit: int = 10) -> List[NeurosynthItem]:
        """Get items."""
    ...
    async def search_items(self, query: str, limit: int = 10) -> List[NeurosynthItem]:
        """Search items."""
        url = f"{BASE_URL}/items"
        params: Dict[str, Any] = {"q": query, "limit": min(limit, 1000)}
        data = await self._rate_limited_get(url, params)
    ...
    async def get_item(self, item_id: str) -> Optional[NeurosynthItem]:
        """Get item by ID."""
        url = f"{BASE_URL}/items/{item_id}"
        data = await self._rate_limited_get(url)
        return self._parse_item(data) if "id" in data else None

    def _parse_dataset(self, data: Dict[str, Any]) -> NeurosynthDataset:
        return NeurosynthDataset(
            id=str(data.get("id", "")),
    ...
    def _parse_item(self, data: Dict[str, Any]) -> NeurosynthItem:
        return NeurosynthItem(
            id=str(data.get("id", "")),
    ...
    async def __aenter__(self) -> "NeurosynthAdapter":
        return self
...
class NeurosynthAPIError(Exception):
    pass


async def _test_neurosynth() -> None:
    adapter = NeurosynthAdapter()
    ...
    await adapter.close()
    print("[TEST] All Neurosynth tests completed.")


if __name__ == "__main__":
    asyncio.run(_test_neurosynth())
