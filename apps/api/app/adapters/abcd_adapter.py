...
ABCD Study adolescent brain imaging dataset.
"""
...
class ABCDAdapter:
    ...
    async def _rate_limited_get(self, url: str, params: Optional[Dict] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            raise ABCDAPIError(f"HTTP {e.response.status_code}: {e.response.text}") from e
        except httpx.RequestError as e:
            raise ABCDAPIError(f"Request failed: {e}") from e

    async def search(self, query: str, limit: int = 10) -> List[ABCDDataset]:
        """Search datasets."""
    ...
    async def get_dataset(self, dataset_id: str) -> Optional[ABCDDataset]:
        """Get dataset by ID."""
    ...
    async def get_datasets(self, limit: int = 10) -> List[ABCDDataset]:
        """List all datasets."""
        url = f"{BASE_URL}/datasets"
    ...
    async def search_by_modality(self, modality: str, limit: int = 10) -> List[ABCDDataset]:
        """Search by modality."""
        url = f"{BASE_URL}/datasets"
    ...
    async def search_by_task(self, task: str, limit: int = 10) -> List[ABCDDataset]:
        """Search by task."""
        url = f"{BASE_URL}/datasets"
    ...
    async def get_recent(self, limit: int = 10) -> List[ABCDDataset]:
        """Get recent datasets."""
        url = f"{BASE_URL}/datasets"
    ...
    async def get_items(self, limit: int = 10) -> List[ABCDItem]:
        """Get items."""
    ...
    async def search_items(self, query: str, limit: int = 10) -> List[ABCDItem]:
        """Search items."""
        url = f"{BASE_URL}/items"
        params: Dict[str, Any] = {"q": query, "limit": min(limit, 1000)}
        data = await self._rate_limited_get(url, params)
    ...
    async def get_item(self, item_id: str) -> Optional[ABCDItem]:
        """Get item by ID."""
        url = f"{BASE_URL}/items/{item_id}"
        data = await self._rate_limited_get(url)
        return self._parse_item(data) if "id" in data else None

    def _parse_dataset(self, data: Dict[str, Any]) -> ABCDDataset:
        return ABCDDataset(
            id=str(data.get("id", "")),
    ...
    def _parse_item(self, data: Dict[str, Any]) -> ABCDItem:
        return ABCDItem(
            id=str(data.get("id", "")),
    ...
    async def __aenter__(self) -> "ABCDAdapter":
        return self
...
class ABCDAPIError(Exception):
    pass


async def _test_abcd() -> None:
    adapter = ABCDAdapter()
    ...
    await adapter.close()
    print("[TEST] All ABCD tests completed.")


if __name__ == "__main__":
    asyncio.run(_test_abcd())
