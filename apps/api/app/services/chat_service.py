...
def _llm_chat(
    ...
    if settings.anthropic_api_key:
        # NB: ``Anthropic`` is imported at module scope so tests can
        # ``monkeypatch.setattr("app.services.chat_service.Anthropic", ...)``;
        # use the module attribute (resolved each call) so the patch wins.
