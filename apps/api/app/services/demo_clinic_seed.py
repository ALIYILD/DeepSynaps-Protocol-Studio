...
# ──────────────────────────────────────────────────────────────────────────────
# Scheduling-focused demo seed (rooms, devices, patients, appointments).
#
# This is intentionally SEPARATE from `seed_demo_clinic_data` (operational
# clinic data) so the Scheduling Hub can be exercised in controlled previews
# without seeding the rest of the operational dataset.
#
# Idempotent: if any demo schedule sessions already exist for the demo
# clinicians in the current week window, this is a no-op.
# ──────────────────────────────────────────────────────────────────────────────


def _iso_naive_utc(dt: datetime) -> str:
    """Normalise a datetime to a naive-UTC ISO string.
    ...
    offset-naive datetimes. Strip tzinfo so demo seed timestamps compare
    consistently with router-created sessions.
    """
...
def seed_demo_clinic(
    ...
    *,
    ...
    window_start = _iso_naive_utc(monday - timedelta(days=1))
    window_end = _iso_naive_utc(monday + timedelta(days=8))
    ...
    # Synthetic schedule: 10 sessions across the current week.
    # NOTE: These are inserted directly (bypassing router conflict validation),
    # because this is a seed script and we intentionally include one conflict
    # pair (demo-sess-005 / demo-sess-006) to exercise conflict surfacing.
    def at(day_offset: int, hour: int, minute: int = 0) -> str:
        return _iso_naive_utc(
            (monday + timedelta(days=day_offset)).replace(hour=hour, minute=minute)
        )
    ...
    for spec in seed_rows:
        ...
        session.add(
            ClinicalSession(
                ...
                session_notes=(
                    f"{_DEMO_SCHED_TAG} Synthetic preview appointment — non-PHI. "
                    "Confirm details in chart."
                ),
                adverse_events=None,
                outcome=None,
                protocol_ref=None,
