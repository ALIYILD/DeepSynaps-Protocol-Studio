...
def get_biomarker_modality_for_fusion(
    patient_id: str,
    actor: AuthenticatedActor = Depends(get_authenticated_actor),
    db: Session = Depends(get_db_session),
) -> dict[str, Any]:
    """Return biomarker modality data formatted for multimodal fusion.

    Returns the most recent lab values for key biomarkers (ferritin,
    vitamin D, TSH, B12) with evidence grades and confidence scores
    for the fusion engine.
    """
    _gate_patient_access(actor, patient_id, db)

    # Fetch latest values for key biomarkers
    key_biomarkers = ["ferritin", "vitamin_d", "tsh", "b12", "crp", "hba1c"]
    features: dict[str, Any] = {}
    abnormal_count = 0

    for bm_id in key_biomarkers:
        row = (
            db.query(PatientLabResult)
            .filter(
                PatientLabResult.patient_id == patient_id,
                PatientLabResult.analyte_code == bm_id,
            )
            .order_by(PatientLabResult.sample_collected_at.desc())
            .first()
        )
        if row and row.value_numeric is not None:
            features[bm_id] = round(float(row.value_numeric), 2)
            if row.ref_low is not None and row.value_numeric < row.ref_low:
                abnormal_count += 1
            if row.ref_high is not None and row.value_numeric > row.ref_high:
                abnormal_count += 1
    ...
    if not features:
        return {
            ...
            "evidence_grade": "C",
            "features": {},
            "safe_summary": "No biomarker data available for this patient.",
    ...
    safe_summary = "Biomarker data available."
    if abnormal_count > 0:
        safe_summary = f"{abnormal_count} biomarker(s) outside reference range. Grade {evidence_grade} evidence."
    else:
        safe_summary = f"All tracked biomarkers within reference range. Grade {evidence_grade} evidence."
