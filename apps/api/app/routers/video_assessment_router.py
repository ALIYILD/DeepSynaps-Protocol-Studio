...
# core-schema-exempt: clinician feedback on advisory summary; not reused outside this router
class HistoricalSummaryFeedbackRequest(BaseModel):
    summary_event_id: str = Field(min_length=8, max_length=64)
    feedback_status: Literal["accepted", "partially_accepted", "disagreed", "not_useful"]
    feedback_note: Optional[str] = Field(default=None, max_length=500)


# core-schema-exempt: compact feedback echo; not reused outside this router
...
def get_historical_ai_summary_feedback(
    session_id: str = PathParam(..., min_length=8),
    summary_event_id: str = PathParam(..., min_length=8),
    actor: AuthenticatedActor = Depends(get_authenticated_actor),
    db: Session = Depends(get_db_session),
) -> HistoricalSummaryFeedbackResponse:
    require_minimum_role(actor, "clinician")
    _summary_event_row_or_404(db, session_id=session_id, summary_event_id=summary_event_id)
    ...
    if payload is None:
        raise ApiServiceError(code="not_found", message="Historical summary feedback not found.", status_code=404)
    return _feedback_response_from_payload(payload)
...
def save_historical_ai_summary_feedback(
    ...
    require_minimum_role(actor, "clinician")
    _summary_event_row_or_404(db, session_id=session_id, summary_event_id=body.summary_event_id)
    feedback_note = _validate_historical_summary_feedback(body)
    payload = _write_historical_summary_feedback_audit(
        db,
        actor=actor,
        session_id=session_id,
