...
_REAL_PATIENT_ANALYZE_SUMMARY = (
    "DeepTwin analyze output is withheld for real patient rows until a validated analysis model is connected."
)
_REAL_PATIENT_SIMULATION_REASON = "no_validated_simulation_engine"
_REAL_PATIENT_SIMULATION_SUMMARY = (
    "DeepTwin simulation output is withheld for real patient rows until a validated simulation engine is connected."
)
_REAL_PATIENT_PROTOCOL_COMPARISON_REASON = "no_validated_protocol_comparison_engine"
_REAL_PATIENT_PROTOCOL_COMPARISON_SUMMARY = (
    "DeepTwin protocol-comparison output is withheld for real patient rows until a validated comparison engine is connected."
)
_REAL_PATIENT_LATENT_REASON = "no_validated_latent_model"
_REAL_PATIENT_LATENT_SUMMARY = (
    "DeepTwin patient latent output is withheld for real patient rows until a validated latent model is connected."
)
...
def _real_patient_analyze_mode_payload(mode: AnalysisMode) -> dict[str, Any]:
    return {
        "mode": mode,
        "available": False,
        "status": "withheld",
        ...
        "limitations": [
            "No validated DeepTwin analyze engine is connected for real patient rows.",
            (
                f"Heuristic exploratory {mode} output is intentionally withheld instead "
                "of being shown as patient-specific guidance."
            ),
            "DeepTwin must not invent patient-specific analysis findings for persisted records.",
        ],
    }


def _real_patient_analyze_payload(payload: DeeptwinAnalyzeRequest) -> DeeptwinAnalyzeResponse:
    requested_modalities = payload.modalities or ["qeeg_features", "assessments", "wearables"]
    limitations = [
        "No validated DeepTwin analyze engine is connected for real patient rows.",
        "Heuristic exploratory analyze output is intentionally withheld instead of returning deterministic placeholders.",
        "DeepTwin must not invent patient-specific analysis findings or latent structure for persisted records.",
    ]
    return DeeptwinAnalyzeResponse(
        ...
        analysis_modes=payload.analysis_modes,
        correlation=(
            _real_patient_analyze_mode_payload("correlation")
            if "correlation" in payload.analysis_modes
            else None
        ),
        prediction=(
            _real_patient_analyze_mode_payload("prediction")
            if "prediction" in payload.analysis_modes
            else None
        ),
        causation=(
            _real_patient_analyze_mode_payload("causation")
            if "causation" in payload.analysis_modes
            else None
        ),
        engine={
            ...
            "notes": [
                "Decision-support only. Real-patient analyze output is withheld until a validated model is connected.",
                "No patient-specific correlation, prediction, or causation payload was emitted.",
            ],
        },
        provenance=build_provenance(
...
def _real_patient_row(session: Session, patient_id: str) -> Any | None:
    return session.get(_DT360_Patient, patient_id)


def _real_patient_dashboard_payload(session: Session, patient_id: str) -> tuple[Any | None, dict[str, Any] | None]:
    patient = _real_patient_row(session, patient_id)
    if patient is None:
        return None, None
