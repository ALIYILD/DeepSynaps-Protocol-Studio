...
class DS030Adapter(BaseAdapter):
    ...
    def fetch(self) -> Dict[str, pathlib.Path]:
        ...
        self._phenotypic_path = part_path
        fetched["participants"] = part_path

        if part_path.exists():
            logger.info("Skipping optional ds000030 metadata downloads during cached/local fetch.")
            return fetched
    ...
    def normalize(self) -> Dict[str, Any]:
        ...
        """
        participants_path = self._participants_path or getattr(self, "_phenotypic_path", None)
        if participants_path is None:
            raise RuntimeError("Call fetch() before normalize().")

        participants = self._load_participants(participants_path)
