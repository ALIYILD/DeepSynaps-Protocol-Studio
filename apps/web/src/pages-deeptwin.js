...
export async function pgDeeptwin(setTopbar /* , navigate */) {
  ...
  if (!patientId) {
    _setMain(`<div class="dt-page">${_renderTabStrip(STATE.activeTab)}${emptyPatientBlock()}${renderSafetyFooter()}</div>`);
    return;
  }
  ...
  try {
    ...
    _setMain(`<div class="dt-page">${_renderTabStrip(STATE.activeTab)}${errorBlock('Failed to load DeepTwin: ' + (e.message || e))}${renderSafetyFooter()}</div>`);
  }
}
