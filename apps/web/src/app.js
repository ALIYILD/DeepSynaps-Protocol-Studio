...
async function init() {
  ...
  try {
    ...
    // Cached session fallback: if we have a cached user in localStorage (from a
    // previous successful auth), attempt to boot with that instead of immediately
    // clearing and navigating away. This handles network failures where /auth/me
    // is temporarily unreachable but the token is still valid.
    try {
      const cachedUserStr = localStorage.getItem('ds_session_user');
      if (cachedUserStr) {
      ...
      // If cached session restore fails, fall through to logout
    }
    api.clearToken();
    navigatePublic('home');
  }
...
(function initCommandPalette() {
  ...
  const NAV_COMMANDS = [
    ...
    { type: 'nav', icon: '◉',  title: 'Assessments',         page: 'assessments-v2' },
  ...
  NAV_COMMANDS.push(
    ...
    { type: 'nav', icon: '🗓️', title: 'Treatment Sessions Analyzer', page: 'treatment-sessions-analyzer' },
    { type: 'nav', icon: '🗓️', title: 'Sessions Adherence', page: 'treatment-sessions-analyzer' },
    { type: 'nav', icon: '🗓️', title: 'Sign-off Queue', page: 'treatment-sessions-analyzer' },
