...
export function renderELoretaROIPanel(analysis) {
  ...
  // CONTRACT shape stores bands under roi_band_power; tolerate legacy bands / flat maps.
  var bandMap = src.roi_band_power || src.bands || src;
...
export function renderQEEGSource3DBrain(analysis) {
  ...
  var caption = '<div class="ds-source-3d-caption">'
    + 'Top ' + TOP_N + ' cortical ROIs by model-estimated band power (MNE minimum-norm source imaging). '
    + 'Dots project approximate regional emphasis onto a 10–20 schematic for orientation — not neuronavigation.'
    + '</div>';

  var dataPayload = encodeURIComponent(JSON.stringify(bandData));

  return card(
    'Source imaging — 3D brain schematic',
...
function _renderComprehensiveReport(report, analysis, savedEvidenceCitations) {
  ...
  // ── Provenance banner: surface when report is a deterministic fallback ────
  // Detect when the LLM was unavailable and the API returned a stub:
  //   - report.model_used / report.model_version is null/empty, AND
  //   - narrative.confidence_level === 'low' (set by _deterministic_fallback)
  // OR when the report explicitly carries source === 'deterministic_stub'.
  var _modelUsed = report && (report.model_used || report.model_version);
  var _reportSource = report && (report.source || report.ai_source || (narrative && narrative.source));
  var _isFallback = (_reportSource === 'deterministic_stub' || _reportSource === 'fallback_no_llm')
    || (!_modelUsed && narrative && narrative.confidence_level === 'low' && Array.isArray(narrative.findings) && narrative.findings.length === 0);
  if (_isFallback) {
    html += '<div role="alert" style="margin-bottom:12px;padding:12px 14px;border-radius:10px;background:rgba(251,191,36,0.10);border:1px solid rgba(251,191,36,0.35);font-size:13px;color:var(--text-primary);line-height:1.55">'
      + '<strong>AI interpretation unavailable.</strong> '
      + 'This report was generated by the deterministic fallback (numerical features + condition matching) — no LLM narrative was produced. '
      + 'Decision-support text below should not be treated as AI-derived.'
      + '</div>';
  }
...
export const TAB_META = {
  ...
  erp:       { label: 'ERP',               color: 'var(--cyan)' },
  report:    { label: 'AI Report',         color: 'var(--violet)' },
  compare:   { label: 'Compare',           color: 'var(--amber)' },
  learning:  { label: 'Learning EEG',      color: 'var(--rose)' },
};

/** Synthetic ERP bundle for demo analysis — not computed by the live pipeline. */
const DEMO_ERP_RESULT = (function () {
  var n = 30;
  var times = [];
  var i;
  for (i = 0; i < n; i++) times.push(-0.1 + (i * 0.6) / (n - 1));
  function chRow(phase) {
    var r = [];
    for (i = 0; i < n; i++) {
      r.push(1.2 * Math.sin(i * 0.4 + phase) + 0.3 * Math.sin(i * 0.15));
    }
    return r;
  }
  return {
    ...
    job_id: 'demo-erp',
    preset: 'p300',
    event_counts: { target: 40, nontarget: 160 },
    evokeds: { target: [chRow(0), chRow(0.2)], nontarget: [chRow(1.1), chRow(0.9)] },
    difference_waves: { target_minus_nontarget: [chRow(0), chRow(0.2)] },
    times: times,
    ch_names: ['Cz', 'Pz'],
    sfreq: 250,
    metrics: {
      metrics: [
        { component: 'P3b', channel: 'Pz', condition: 'target', mean_amplitude_uv: 8.2, peak_amplitude_uv: 12.1, peak_latency_s: 0.35, snr: 4.1 },
        { component: 'P3a', channel: 'Cz', condition: 'target', mean_amplitude_uv: 4.0, peak_latency_s: 0.25, snr: 2.4 },
      ],
      qc: {},
      notes: [],
    },
    warnings: ['LOW_TRIAL_COUNT:target'],
    caveats: [
      'ERP metrics depend on task validity, marker timing, preprocessing, and adequate trials per condition.',
      'Not diagnostic by itself — requires clinician review and clinical context.',
    ],
    interpretation: [
      'Clinician review is required before using these metrics in care decisions.',
      'P300 amplitude/latency patterns are task- and population-dependent; reduced amplitude is not specific to any diagnosis.',
    ],
    clinician_review_required: true,
    reject_qc: { n_dropped_amplitude: 2, n_after: 198 },
  };
})();

function _erpSvgEvoked(times, evokeds, diffWaves, chIdx) {
  var w = 440;
  var h = 132;
  var pad = 10;
  if (!times || !times.length) return '';
  var series = {};
  if (evokeds && typeof evokeds === 'object') {
    Object.keys(evokeds).forEach(function (cond) {
      var M = evokeds[cond];
      if (Array.isArray(M) && M[chIdx] && Array.isArray(M[chIdx])) series[cond] = M[chIdx];
    });
  }
  if (diffWaves && typeof diffWaves === 'object') {
    Object.keys(diffWaves).forEach(function (dk) {
      var M = diffWaves[dk];
      if (Array.isArray(M) && M[chIdx] && Array.isArray(M[chIdx])) series['Δ ' + dk] = M[chIdx];
    });
  }
  ...
  keys.forEach(function (k) {
    series[k].forEach(function (v) { all.push(Number(v)); });
  });
  var minT = times[0];
  var maxT = times[times.length - 1];
  var minY = Math.min.apply(null, all.concat([0]));
  var maxY = Math.max.apply(null, all.concat([1]));
  var yR = (maxY - minY) || 1;
  var yPad = yR * 0.1;
  minY -= yPad;
  maxY += yPad;
  function xScale(t) {
    return pad + ((t - minT) / (maxT - minT || 1)) * (w - 2 * pad);
  }
  function yScale(v) {
    return h - pad - ((v - minY) / (maxY - minY || 1)) * (h - 2 * pad);
  }
  var colors = ['#38bdf8', '#f472b6', '#a78bfa', '#34d399', '#fbbf24'];
  var paths = [];
  keys.forEach(function (k, ki) {
    var row = series[k];
    var d = '';
    for (var i = 0; i < times.length; i++) {
      var x = xScale(times[i]);
      var y = yScale(Number(row[i]));
      d += (i === 0 ? 'M' : 'L') + x.toFixed(2) + ',' + y.toFixed(2) + ' ';
    }
    paths.push('<path fill="none" stroke="' + colors[ki % colors.length] + '" stroke-width="1.5" d="' + esc(d.trim()) + '" />');
  });
  return '<svg data-testid="qeeg-erp-waveform-svg" viewBox="0 0 ' + w + ' ' + h + '" width="100%" style="max-height:200px;background:rgba(255,255,255,0.02);border-radius:8px;border:1px solid var(--border)">' + paths.join('') + '</svg>';
}
...
export async function pgQEEGAnalysis(setTopbar, navigate) {
  ...
  var _heroSrcLocBtn = document.getElementById('qeeg-hero-source-loc');
  if (_heroSrcLocBtn) {
    _heroSrcLocBtn.addEventListener('click', function () {
      if (!heroAnalysisId) {
        showToast('Select or upload a recording first', 'warning');
        return;
      }
      try {
        window._qeegSourceLocAnalysisId = heroAnalysisId;
        window._qeegAudit('open_source_localization', { analysis_id: heroAnalysisId });
      } catch (_e) {}
      window.location.hash = '#/qeeg-source-localization/' + encodeURIComponent(heroAnalysisId);
      if (typeof window._nav === 'function') window._nav('qeeg-source-localization');
    });
  }
  ...
  if (tab === 'report') {
    ...
    try {
      ...
      // Demo mode fallback when no reports available — only swap in the demo
      // payload when the analysis itself is the synthetic 'demo' record AND
      // the build was flagged as demo. Audit-log the swap so a clinician
      // reviewing audit history can tell that the rendered report came from
      // a demo seed, not a real LLM call.
      if (reports.length === 0 && _isDemoMode() && analysisId === 'demo') {
        try { _qeegAudit('ai_report_demo_fallback', { analysis_id: analysisId, source: 'demo_seed' }); } catch (_e) {}
  ...
  // ══════════════════════════════════════════════════════════════════════════
  // TAB: ERP (event-related potentials) — never uses resting fixed-length epochs
  // ══════════════════════════════════════════════════════════════════════════
  if (tab === 'erp') {
    var erpAnalysisId = window._qeegSelectedId;
    if (!erpAnalysisId) {
      tabEl.innerHTML =
        '<div style="max-width:640px;margin:48px auto;padding:32px;border-radius:14px;background:rgba(255,255,255,0.02);border:1px solid rgba(255,255,255,0.06);text-align:center">'
        + '<div style="font-size:32px;margin-bottom:8px">&#9889;</div>'
        + '<div style="font-size:18px;font-weight:700;margin-bottom:6px">Select a recording</div>'
        + '<div style="font-size:13px;color:var(--text-secondary);line-height:1.6;margin-bottom:18px">ERP analysis needs an uploaded session with event markers (or an explicit events array). Choose an analysis from Patient &amp; Upload.</div>'
        + '<button class="btn btn-outline btn-sm" onclick="window._qeegTab=\'patient\';window._nav(\'qeeg-analysis\')">Patient &amp; Upload</button>'
        + '</div>';
      return;
    }

    if (!(erpAnalysisId === 'demo' && _isDemoMode())) {
      if (!_currentAnalysis || _currentAnalysis.id !== erpAnalysisId) {
        try {
          _currentAnalysis = await api.getQEEGAnalysis(erpAnalysisId);
        } catch (_e) {
          /* ERP tab still usable without full analysis payload */
        }
      }
    }

    var erpHtml = '';
    erpHtml += renderLaunchNotice(
      'Decision-support only — ERP',
      'Event-related potentials require valid triggers aligned to your task. This module never falls back to resting-state epochs. Interpretation depends on task design, trial counts, and artifact handling.',
      'warn'
    );

    var isDemoErp = erpAnalysisId === 'demo' && _isDemoMode();
    var demoResult = DEMO_ERP_RESULT;
    var lastResult = window._qeegErpLastResult || null;

    var bidsMeta = erpResolveBidsUploadMeta(
      erpAnalysisId,
      _currentAnalysis,
      typeof window !== 'undefined' ? window._qeegErpLastBidsUpload : null
    );

    function _erpFormatMappingTable(bm) {
      if (!bm || !bm.trial_types || !bm.trial_types.length) {
        return '<p data-testid="qeeg-erp-bids-mapping-empty" style="font-size:13px;color:var(--text-secondary)">No sidecar trial types yet. Upload a BIDS file, then assign trigger codes (optional helper).</p>';
      }
      var rows = bm.trial_types
        .map(function (t, idx) {
          var code = idx + 1;
          return (
            '<tr data-erp-map-row="">' +
            '<td style="font-size:12px;vertical-align:middle"><code>' +
            esc(String(t)) +
            '</code></td>' +
            '<td><input type="text" class="form-control form-control-sm" data-erp-map-key="" aria-label="Map key for trial type" value="' +
            esc(String(t)) +
            '" style="min-width:120px" /></td>' +
            '<td style="width:92px"><input type="number" class="form-control form-control-sm" data-erp-map-code="" value="' +
            code +
            '" /></td>' +
            '</tr>'
          );
        ...
        .join('');
      return (
        '<div data-testid="qeeg-erp-bids-mapping">' +
        '<p style="font-size:12px;color:var(--text-secondary);margin:0 0 8px;line-height:1.5">Each <strong>map key</strong> must match the corresponding BIDS <code>trial_type</code> cell (unless normalization options were used on upload). Codes become integer triggers for epoching.</p>' +
        '<table class="table" style="font-size:13px;width:100%;max-width:560px"><thead><tr><th>Detected</th><th>Map key</th><th>Code</th></tr></thead><tbody>' +
        rows +
        '</tbody></table>' +
        '<button type="button" class="btn btn-outline btn-sm" id="qeeg-erp-bids-apply-map" data-testid="qeeg-erp-bids-apply-map" style="margin-top:10px">Apply mapping to JSON below</button>' +
        '</div>'
      );
    }
    ...
    erpHtml += card(
      'ERP setup',
      '<div data-testid="qeeg-erp-setup" style="display:grid;gap:12px;max-width:720px">' +
        '<label style="font-size:12px;font-weight:600">Preset</label>' +
        '<select id="qeeg-erp-preset" data-testid="qeeg-erp-preset" class="form-select" style="max-width:280px">' +
        '<option value="p300">P300</option><option value="mmn">MMN</option><option value="custom">Custom</option>' +
        '</select>' +
        '<label style="font-size:12px;font-weight:600">Stimulus channel (optional if BIDS sidecar, upload below, or events pasted)</label>' +
        '<input id="qeeg-erp-stim" class="form-control" placeholder="e.g. STI 014" />' +
        '<div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">' +
        '<div><label class="form-label" style="font-size:11px">tmin (s)</label><input id="qeeg-erp-tmin" class="form-control" value="-0.1" /></div>' +
        '<div><label class="form-label" style="font-size:11px">tmax (s)</label><input id="qeeg-erp-tmax" class="form-control" value="0.5" /></div>' +
        '</div>' +
        '<label style="font-size:12px;font-weight:600">BIDS events.tsv (optional)</label>' +
        '<div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center;font-size:13px">' +
        '<input type="file" id="qeeg-erp-bids-file" data-testid="qeeg-erp-bids-file" accept=".tsv,text/tab-separated-values" />' +
        '<button type="button" class="btn btn-outline btn-sm" id="qeeg-erp-bids-upload" data-testid="qeeg-erp-bids-upload">Upload sidecar</button>' +
        '<span id="qeeg-erp-bids-status" data-testid="qeeg-erp-bids-status" style="font-size:12px;color:var(--text-secondary)"></span>' +
        '</div>' +
        '<div style="display:flex;flex-wrap:wrap;gap:12px;font-size:12px;align-items:center">' +
        '<label style="display:inline-flex;gap:6px;align-items:center;cursor:pointer"><input type="checkbox" id="qeeg-erp-bids-trim" data-testid="qeeg-erp-bids-trim" /> Trim <code>trial_type</code> whitespace</label>' +
        '<label style="display:inline-flex;gap:6px;align-items:center;cursor:pointer"><input type="checkbox" id="qeeg-erp-bids-lower" data-testid="qeeg-erp-bids-lower" /> Lower-case <code>trial_type</code> for matching</label>' +
        '</div>' +
        '<div style="font-size:11px;color:var(--text-secondary);line-height:1.5">Stores ``{recording_stem}_events.tsv`` on the server next to the uploaded EEG. Requires UTF-8; rows validated before save (<strong>onset</strong> ≥ 0, non-empty <strong>trial_type</strong>, optional <strong>duration</strong>). Normalization checkboxes default off (strict BIDS). ERP resolution order: pasted events → stim channel → this file.</div>' +
        '</div>'
    );

    erpHtml += card(
      'BIDS sidecar & condition mapping',
      '<div style="display:grid;gap:14px;max-width:720px">' +
        erpFormatBidsSummaryHtml(bidsMeta) +
        '<hr style="margin:0;border:none;border-top:1px solid var(--border)" />' +
        _erpFormatMappingTable(bidsMeta) +
        '<label style="font-size:12px;font-weight:600">Event ID map (JSON) — condition label → trigger integer</label>' +
        '<textarea id="qeeg-erp-map" data-testid="qeeg-erp-map" class="form-control" rows="4" spellcheck="false">{"c1":1,"c2":2}</textarea>' +
        '<div style="font-size:11px;color:var(--text-secondary)">Advanced: edit JSON directly; helper overwrites this textarea when you click <strong>Apply mapping</strong>.</div>' +
        '<div id="qeeg-erp-map-run-warnings" data-testid="qeeg-erp-map-run-warnings" aria-live="polite"></div>' +
        '<label style="font-size:12px;font-weight:600">Optional events array (JSON) — rows [sample, prev, code]</label>' +
        '<textarea id="qeeg-erp-events" class="form-control" rows="3" spellcheck="false" placeholder="[[100,0,1],[200,0,2]]"></textarea>' +
        '<div style="font-size:11px;color:var(--text-secondary)">When set, explicit events override the sidecar at run time.</div>' +
        '<div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center">' +
        '<button type="button" class="btn btn-primary btn-sm" id="qeeg-erp-run" data-testid="qeeg-erp-run">Run ERP analysis</button>' +
        '<span id="qeeg-erp-run-status" role="status" style="font-size:12px;color:var(--text-secondary)"></span>' +
        '</div>' +
        '<div id="qeeg-erp-unavailable" data-testid="qeeg-erp-unavailable" style="display:none;margin-top:8px;padding:10px;border-radius:8px;background:rgba(239,68,68,0.08);border:1px solid rgba(239,68,68,0.35);font-size:13px;color:var(--text-primary)"></div>' +
        '</div>'
    );

    function _erpMetricsTable(res) {
      var pack = res && res.metrics;
      var rows = (pack && pack.metrics) || [];
      if (!rows.length) return '<p style="font-size:13px;color:var(--text-secondary)">No component metrics returned.</p>';
      var head = '<thead><tr><th>Component</th><th>Channel</th><th>Condition</th><th>Latency (s)</th><th>Amplitude (µV)</th><th>SNR</th></tr></thead>';
      var body = '<tbody>' + rows.map(function (r) {
        return '<tr><td>' + esc(String(r.component || '')) + '</td><td>' + esc(String(r.channel || '')) + '</td><td>' + esc(String(r.condition || '')) + '</td><td>'
          + esc(String(r.peak_latency_s != null ? r.peak_latency_s : '')) + '</td><td>'
          + esc(String(r.peak_amplitude_uv != null ? r.peak_amplitude_uv : r.mean_amplitude_uv || '')) + '</td><td>' + esc(String(r.snr != null ? r.snr : '')) + '</td></tr>';
      }).join('') + '</tbody>';
      return '<table data-testid="qeeg-erp-metrics-table" class="table" style="font-size:13px;width:100%">' + head + body + '</table>';
    }

    function _erpEventQc(res) {
      var ec = res && res.event_counts;
      if (!ec || typeof ec !== 'object') {
        return '<p style="font-size:13px;color:var(--text-secondary)">No per-condition counts.</p>';
      }
      var parts = Object.keys(ec).map(function (k) {
        return '<div><strong>' + esc(k) + ':</strong> ' + esc(String(ec[k])) + ' trials</div>';
      });
      return '<div data-testid="qeeg-erp-event-qc" style="font-size:13px;line-height:1.7">' + parts.join('') + '</div>';
    }

    function _erpGuardrails() {
      return '<div data-testid="qeeg-erp-guardrails" style="margin-top:16px;padding:14px;border-radius:10px;background:var(--surface-tint-1);border:1px solid var(--border);font-size:12px;line-height:1.65;color:var(--text-secondary)">'
        + '<div style="font-weight:700;color:var(--amber);margin-bottom:8px">Clinical interpretation guardrails</div>'
        + '<ul style="margin:0;padding-left:18px">'
        + '<li><strong>Decision-support only</strong> — not a standalone diagnostic.</li>'
        + '<li><strong>Requires clinician review</strong> before use in care decisions.</li>'
        + '<li>Results depend on <strong>task design</strong>, event timing, artifacts, and accepted trial counts.</li>'
        + '<li><strong>Not diagnostic by itself</strong> — combine with history, examination, and raw EEG review.</li>'
        + '</ul></div>';
    }

    var displayRes = isDemoErp ? demoResult : lastResult;
    if (displayRes && displayRes.status === 'completed') {
      var chSelIdx = 0;
      var chNames = displayRes.ch_names || [];
      erpHtml += card('Event QC', _erpEventQc(displayRes)
        + (displayRes.warnings && displayRes.warnings.length
          ? '<div style="margin-top:10px;font-size:12px;color:var(--amber)"><strong>QC:</strong> ' + esc(displayRes.warnings.join('; ')) + '</div>'
          : ''));
      erpHtml += card('ERP waveforms',
        '<label style="font-size:12px;font-weight:600">Channel</label>'
        + '<select id="qeeg-erp-ch" data-testid="qeeg-erp-channel" class="form-select" style="max-width:200px;margin-bottom:10px">'
        + chNames.map(function (c, i) { return '<option value="' + i + '">' + esc(c) + '</option>'; }).join('')
        + '</select>'
        + '<div id="qeeg-erp-wave-host">' + _erpSvgEvoked(displayRes.times, displayRes.evokeds, null, chSelIdx) + '</div>');
      if (displayRes.difference_waves && Object.keys(displayRes.difference_waves).length) {
        erpHtml += card('Difference waves', '<div style="font-size:12px;color:var(--text-secondary);margin-bottom:8px">Contrasts from pipeline configuration (e.g. target − standard).</div>'
          + '<div id="qeeg-erp-diff-host">' + _erpSvgEvoked(displayRes.times, {}, displayRes.difference_waves, chSelIdx) + '</div>');
      }
      erpHtml += card('Component metrics', _erpMetricsTable(displayRes));
      if (displayRes.interpretation && displayRes.interpretation.length) {
        erpHtml += card('Deterministic interpretation (rules-first)',
          '<ul style="font-size:13px;line-height:1.65;margin:0;padding-left:18px">' + displayRes.interpretation.map(function (line) {
            return '<li>' + esc(line) + '</li>';
          }).join('') + '</ul>');
      }
    } else if (!isDemoErp) {
      erpHtml += card('Results', '<p data-testid="qeeg-erp-no-result" style="font-size:13px;color:var(--text-secondary)">Run an ERP job to see event QC, waveforms, and metrics. If the recording has no event markers, the API returns a clear error — resting qEEG epochs are never used.</p>');
    }

    erpHtml += _erpGuardrails();

    if (isDemoErp) {
      erpHtml = _demoBanner() + erpHtml;
    }

    tabEl.innerHTML = erpHtml;

    function _erpRefreshMapWarnings() {
      var warnEl = document.getElementById('qeeg-erp-map-run-warnings');
      var mapEl = document.getElementById('qeeg-erp-map');
      var evEl = document.getElementById('qeeg-erp-events');
      if (!warnEl || !mapEl) return;
      var evTxt = evEl && evEl.value.trim();
      if (evTxt) {
        warnEl.innerHTML =
          '<div style="font-size:12px;color:var(--text-secondary)" data-testid="qeeg-erp-mapping-skip-note">Explicit events override the sidecar at run time — BIDS mapping checks skipped.</div>';
        return;
      }
      var parsed = {};
      try {
        parsed = JSON.parse(mapEl.value || '{}');
      } catch (_e) {
        warnEl.innerHTML =
          '<div data-testid="qeeg-erp-map-json-error" style="font-size:12px;color:var(--amber)">event_id_map JSON is not valid.</div>';
        return;
      }
      var bm = window._qeegErpLastBidsUpload;
      var tt =
        bm && bm.analysisId === erpAnalysisId && bm.trial_types ? bm.trial_types : [];
      var msgs = erpValidateEventMapping(tt, parsed);
      if (!msgs.length) {
        warnEl.innerHTML = '';
        return;
      }
      warnEl.innerHTML =
        '<div data-testid="qeeg-erp-mapping-warnings" style="font-size:12px;color:var(--amber);margin-top:4px"><strong>Mapping check (review before run):</strong><ul style="margin:6px 0 0;padding-left:18px">' +
        msgs
          .map(function (m) {
            return '<li>' + esc(m) + '</li>';
          })
    ...
    var mapTa = document.getElementById('qeeg-erp-map');
    var evTa = document.getElementById('qeeg-erp-events');
    if (mapTa && !isDemoErp) mapTa.addEventListener('input', _erpRefreshMapWarnings);
    if (evTa && !isDemoErp) evTa.addEventListener('input', _erpRefreshMapWarnings);
    _erpRefreshMapWarnings();

    var chSel = document.getElementById('qeeg-erp-ch');
    function _refreshErpPlots() {
      var idx = chSel ? parseInt(chSel.value, 10) || 0 : 0;
      var resPlot = isDemoErp ? demoResult : (window._qeegErpLastResult || null);
      if (!resPlot || resPlot.status !== 'completed') return;
      var wh = document.getElementById('qeeg-erp-wave-host');
      var dh = document.getElementById('qeeg-erp-diff-host');
      if (wh) wh.innerHTML = _erpSvgEvoked(resPlot.times, resPlot.evokeds, null, idx);
      if (dh) dh.innerHTML = _erpSvgEvoked(resPlot.times, {}, resPlot.difference_waves, idx);
    }
    if (chSel) chSel.addEventListener('change', _refreshErpPlots);

    var bidsUp = document.getElementById('qeeg-erp-bids-upload');
    var bidsFile = document.getElementById('qeeg-erp-bids-file');
    var bidsSt = document.getElementById('qeeg-erp-bids-status');
    var bidsTrim = document.getElementById('qeeg-erp-bids-trim');
    var bidsLower = document.getElementById('qeeg-erp-bids-lower');
    if (bidsUp && bidsFile && !isDemoErp) {
      bidsUp.addEventListener('click', async function () {
        var f = bidsFile.files && bidsFile.files[0];
        if (!f) {
          if (bidsSt) bidsSt.textContent = 'Choose a .tsv file first';
          return;
        }
        if (bidsSt) bidsSt.textContent = 'Uploading…';
        try {
          var up = await api.uploadQEEGERPBidsEvents(erpAnalysisId, f, {
            normalizeTrim: bidsTrim && bidsTrim.checked,
            normalizeCaseLower: bidsLower && bidsLower.checked,
          });
          window._qeegErpLastBidsUpload = {
            analysisId: erpAnalysisId,
            row_count: up.row_count,
            trial_types: up.trial_types || [],
            warnings: up.warnings || [],
            normalized: !!up.normalized,
            sidecar_ref: up.sidecar_ref,
            bytes_written: up.bytes_written,
          };
          if (bidsSt) {
            var rc = up && up.row_count != null ? up.row_count : '?';
            var tt = up && up.trial_types && up.trial_types.length ? up.trial_types.join(', ') : '';
            var warn = up && up.warnings && up.warnings.length ? ' Warnings: ' + up.warnings.length + '.' : '';
            bidsSt.textContent = 'Saved (' + rc + ' rows' + (tt ? '; trial_type: ' + tt : '') + ')' + warn + ' Review mapping below.';
          }
          ...
          if (bidsSt) bidsSt.textContent = esc(String(err && err.message ? err.message : err));
          showToast('BIDS upload failed: ' + (err.message || err), 'error');
        }
      });
    }

    var applyMapBtn = document.getElementById('qeeg-erp-bids-apply-map');
    if (applyMapBtn && !isDemoErp) {
      applyMapBtn.addEventListener('click', function () {
        var mapOut = document.getElementById('qeeg-erp-map');
        var trs = document.querySelectorAll('tr[data-erp-map-row]');
        var rows = [];
        trs.forEach(function (tr) {
          var kInp = tr.querySelector('[data-erp-map-key]');
          var cInp = tr.querySelector('[data-erp-map-code]');
          if (kInp && cInp) {
            rows.push({ conditionKey: kInp.value, code: cInp.value });
          }
        });
        var o = erpApplyTrialMappingRows(rows);
        if (mapOut) mapOut.value = JSON.stringify(o);
        _erpRefreshMapWarnings();
      });
    }

    var runBtn = document.getElementById('qeeg-erp-run');
    if (runBtn && !isDemoErp) {
      runBtn.addEventListener('click', async function () {
        _erpRefreshMapWarnings();
        var stEl = document.getElementById('qeeg-erp-run-status');
        var badEl = document.getElementById('qeeg-erp-unavailable');
        if (badEl) { badEl.style.display = 'none'; badEl.textContent = ''; }
        try {
          var presetEl = document.getElementById('qeeg-erp-preset');
          var mapTxt = (document.getElementById('qeeg-erp-map') && document.getElementById('qeeg-erp-map').value) || '{}';
          var evTxt = (document.getElementById('qeeg-erp-events') && document.getElementById('qeeg-erp-events').value.trim()) || '';
          var body = {
            preset: presetEl ? presetEl.value : 'custom',
            event_id_map: JSON.parse(mapTxt),
            tmin: parseFloat(document.getElementById('qeeg-erp-tmin').value) || -0.1,
            tmax: parseFloat(document.getElementById('qeeg-erp-tmax').value) || 0.5,
            clinician_review_required: true,
          };
          var stim = (document.getElementById('qeeg-erp-stim') && document.getElementById('qeeg-erp-stim').value.trim());
          if (stim) body.stim_channel = stim;
          if (evTxt) body.events = JSON.parse(evTxt);
          if (stEl) stEl.textContent = 'Submitting…';
          var sub = await api.runQEEGERP(erpAnalysisId, body);
          var jobId = sub && sub.job_id;
          if (!jobId) throw new Error('No job_id returned');
          var status = 'pending';
          for (var attempt = 0; attempt < 90; attempt++) {
            var st = await api.getQEEGERPStatus(erpAnalysisId, jobId);
            status = st && st.status;
          ...
          if (status === 'failed') {
            ...
            if (badEl) {
              ...
              badEl.textContent = 'ERP unavailable: run failed or no valid event markers. Resting qEEG segmentation is not used.';
            }
            try { _qeegAudit('erp_run_failed', { analysis_id: erpAnalysisId, job_id: jobId }); } catch (_e) {}
            return;
          }
          if (status !== 'completed') {
            if (stEl) stEl.textContent = 'Timed out waiting for ERP job';
            return;
          }
          var res = await api.getQEEGERPResult(erpAnalysisId, jobId);
          window._qeegErpLastResult = res;
          try { _qeegAudit('erp_result_opened', { analysis_id: erpAnalysisId, job_id: jobId }); } catch (_e) {}
          window._qeegTab = 'erp';
          if (typeof window._nav === 'function') window._nav('qeeg-analysis');
        } catch (err) {
          if (stEl) stEl.textContent = '';
          if (badEl) {
            badEl.style.display = 'block';
            badEl.textContent = 'ERP unavailable: ' + esc(String(err && err.message ? err.message : err));
          }
          showToast('ERP run failed: ' + (err.message || err), 'error');
        }
      });
    }
    if (isDemoErp && runBtn) {
      runBtn.disabled = true;
      runBtn.title = 'Demo session uses offline ERP preview only';
    }
