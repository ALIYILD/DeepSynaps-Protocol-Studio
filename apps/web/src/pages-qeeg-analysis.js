...
function _renderComprehensiveReport(report, analysis, savedEvidenceCitations) {
  ...
  var html = '';
  var syntheticDemo = _qeegReportIsSyntheticDemo(report, analysis);
  ...
  if (syntheticDemo) {
    html += '<div data-testid="qeeg-report-synthetic-banner" role="status" aria-live="polite" style="margin-bottom:12px;padding:12px 14px;border-radius:10px;background:rgba(251,191,36,0.12);border:1px solid rgba(251,191,36,0.45);font-size:13px;color:var(--text-primary);line-height:1.55">'
      + '<strong>Synthetic demo preview.</strong> Numbers, maps, and narratives below illustrate the report layout only. '
      + 'They are not patient-specific findings. Condition-match bars, protocol suggestion lists, illustrative spike sharp-wave tables, '
      + 'and condition-likelihood gauges are hidden here so the preview cannot be mistaken for a clinical diagnosis or protocol prescription.'
      + '</div>';
  }
  ...
  // Synthetic demo bundles include illustrative spike/sharp-wave counts — do not present as clinical epileptiform detection.
  if (analysis && analysis.pathological_signs && !syntheticDemo) {
    var pathHtml = '';
  ...
  if (analysis && analysis.brodmann_areas && !syntheticDemo) {
    var brodHtml = '';
  ...
  if (analysis && analysis.biomarkers && !syntheticDemo) {
    var bioHtml = '';
  ...
  if (conditions.length && !syntheticDemo) {
    ...
    condHtml += '</div>';
    html += card('Condition pattern similarity (decision-support)', condHtml
      + '<div style="margin-top:10px;font-size:11px;color:var(--text-tertiary)">These bars reflect mathematical similarity to named clinical profiles in the model — <strong>not</strong> a psychiatric or neurological diagnosis.</div>');
  }

  // ── Section 12: Protocol Suggestions ────────────────────────────────────
  if (suggestions.length && !syntheticDemo) {
...
var DEMO_ANALYSIS_ENTRY = {
  id: 'demo', is_synthetic_demo: true, analysis_status: 'completed', original_filename: 'demo_eyes_closed.edf',
...
/** True when analysis payload is the bundled preview / synthetic demo shape (not a persisted clinical run). */
export function _qeegAnalysisIsSyntheticDemo(analysis) {
  if (!analysis || typeof analysis !== 'object') return false;
  if (analysis.id === 'demo') return true;
  if (analysis.is_demo === true || analysis.is_synthetic_demo === true) return true;
  if (analysis.demo_source === 'preview_bundle') return true;
  if (analysis.norm_db_version === 'toy-0.1') return true;
  return false;
}

/** True when the paired report is synthetic demo content (must not be presented as clinical findings). */
export function _qeegReportIsSyntheticDemo(report, analysis) {
  if (_qeegAnalysisIsSyntheticDemo(analysis)) return true;
  if (!report || typeof report !== 'object') return false;
  if (report.id === 'demo-report') return true;
  if (report.is_demo === true) return true;
  return false;
}
