...
test('pgFinanceHub view-only invoice path blocks invoice creation', async () => {
  const dom = installDomHarness();
  const originalUser = authMod.currentUser;
  const originals = {
    summary: api.finance.summary,
    listInvoices: api.finance.listInvoices,
    listPayments: api.finance.listPayments,
    listClaims: api.finance.listClaims,
    monthlyAnalytics: api.finance.monthlyAnalytics,
  };

  authMod.setCurrentUser({ role: 'clinician' });
  Object.assign(api.finance, sampleFinanceApi());
  ...
  try {
    ...
    assert.equal(topbarTitle, 'Finance');
    assert.match(topbarActions, /View-only/);
    assert.match(globalThis.document.getElementById('content').innerHTML, /Invoices/);

    window._finNewInvoice();
    assert.equal(dom.toastCalls.at(-1).title, 'View only');
    assert.match(dom.toastCalls.at(-1).body, /clinic admin/i);
  } finally {
    api.finance.summary = originals.summary;
    api.finance.listInvoices = originals.listInvoices;
    api.finance.listPayments = originals.listPayments;
    api.finance.listClaims = originals.listClaims;
    api.finance.monthlyAnalytics = originals.monthlyAnalytics;
    authMod.setCurrentUser(originalUser);
    dom.restore();
  }
});
