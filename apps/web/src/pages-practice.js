...
<div style="max-width:960px;margin:0 auto;padding:0 4px">
  ...
  // ── Download (recording upload is not wired) ───────────────────────────────
  // There is no backend endpoint for telehealth recording upload in this
  // deployment. The previous implementation ran a setTimeout and marked the
  // record as "uploaded" without ever sending bytes anywhere — that has been
  // replaced with an honest local download so the clinician can archive the
  // file via existing document workflows.
  window._recUpload = function() {
    const id = window._lastSavedRecId;
    if (!id) { _recShowToast('No recording to save.', 'error'); return; }
    const recs = getRecordings();
    const rec = recs.find(r => r.id === id);
    if (!rec) { _recShowToast('Recording not found.', 'error'); return; }
    if (!rec.blobUrl) {
      _recShowToast('Recording unavailable — nothing to download.', 'error');
      return;
    }
    try {
      const a = document.createElement('a');
      a.href = rec.blobUrl;
      const safe = (rec.patientName || 'session').replace(/[^a-z0-9\-]+/gi, '-');
      a.download = `telehealth-${safe}-${rec.id}.webm`;
      document.body.appendChild(a);
      a.click();
      a.remove();
      _recShowToast('Download started — cloud upload not configured.', 'info');
    } catch {
      _recShowToast('Could not start download.', 'error');
    }
  };
...
export async function pgClinicAcademy(setTopbar, _currentUser) {
  ...
  const SECTIONS = [
    ...
    },
    {
      ...
      sub: 'Self-paced online courses from Coursera, edX, Udemy, and FutureLearn — fundamentals through advanced protocol design.',
      items: [
        { name: 'Medical Neuroscience (Duke)',  org: 'Coursera',          desc: 'Comprehensive neuroanatomy + neurophysiology fundamentals — recommended prereq for stimulation work.', href: 'https://www.coursera.org/learn/medical-neuroscience', tag: 'Coursera · Free audit' },
        ...
        { name: 'Computational Neuroscience',   org: 'UW · Coursera',     desc: 'Good primer on neural coding and modelling — useful for closed-loop stimulation design.', href: 'https://www.coursera.org/learn/computational-neuroscience', tag: 'Coursera · ~9 weeks' },
        { name: 'Neurohacking in R',            org: 'Johns Hopkins · Coursera', desc: 'Neuroimaging analysis in R — preprocessing, structural + functional MRI.', href: 'https://www.coursera.org/learn/neurohacking', tag: 'Coursera · Imaging' },
        { name: 'Brain Stimulation Primer',     org: 'FutureLearn',       desc: 'Short introductory course on TMS and tDCS for clinicians and researchers.', href: 'https://www.futurelearn.com/subjects/healthcare-medicine-courses/mental-health', tag: 'FutureLearn · Intro' },
        ...
        { name: 'Introduction to Psychology',    org: 'Yale · Coursera',   desc: 'Paul Bloom\'s seminal course covering brain structure, neural development, perception, learning, memory.', href: 'https://www.coursera.org/learn/introduction-psychology', tag: 'Coursera · Free audit' },
        { name: 'Understanding the Brain',       org: 'University of Chicago · Coursera', desc: 'The neurobiology of everyday life — how the brain generates behaviour and how disease affects it.', href: 'https://www.coursera.org/learn/neurobiology', tag: 'Coursera · Free audit' },
        { name: 'The Brain and Space',           org: 'Duke · Coursera',   desc: 'How the brain creates our sense of spatial awareness. Spatial perception, sensory systems, brain mapping.', href: 'https://www.coursera.org/learn/the-brain-and-space', tag: 'Coursera · Free audit' },
        { name: 'Neuroscience of Leadership',    org: 'Udemy',             desc: 'Apply neuroscience research to leadership, decision-making, and change management in clinical settings.', href: 'https://www.udemy.com/topic/neuroscience/', tag: 'Udemy · On-demand' },
        { name: 'Cellular Mechanisms of Brain Function', org: 'EPFL · edX', desc: 'Deep dive into neurons, synapses, and neural circuits. From the Brain Mind Institute at EPFL.', href: 'https://www.edx.org/learn/neuroscience/ecole-polytechnique-federale-de-lausanne-cellular-mechanisms-of-brain-function', tag: 'edX · Advanced' },
        { name: 'Positive Psychiatry and Mental Health', org: 'University of Sydney · Coursera', desc: 'Evidence-based approaches to positive psychiatry — complements neuromodulation intervention planning.', href: 'https://www.coursera.org/learn/positive-psychiatry', tag: 'Coursera · Free audit' },
      ],
    },
    {
