/**
 * Intervention Analyzer — clinician-reviewed intervention decision support.
...
const INTERVENTION_TYPES = [
  { slug: 'tms', label: 'TMS', category: 'neuromodulation' },
  { slug: 'tdcs', label: 'tDCS', category: 'neuromodulation' },
  { slug: 'tacs', label: 'tACS', category: 'neuromodulation' },
  { slug: 'trns', label: 'tRNS', category: 'neuromodulation' },
  { slug: 'tavns', label: 'taVNS', category: 'neuromodulation' },
  { slug: 'tps', label: 'TPS', category: 'neuromodulation' },
  { slug: 'pbm', label: 'PBM', category: 'neuromodulation' },
  { slug: 'neurofeedback', label: 'Neurofeedback', category: 'neuromodulation' },
  { slug: 'medication_change', label: 'Medication change', category: 'pharmacological' },
  { slug: 'psychotherapy', label: 'Psychotherapy', category: 'behavioural' },
  { slug: 'occupational_therapy', label: 'Occupational therapy', category: 'behavioural' },
  { slug: 'speech_therapy', label: 'Speech/language therapy', category: 'behavioural' },
  { slug: 'physiotherapy', label: 'Physiotherapy', category: 'behavioural' },
  { slug: 'digital_therapeutics', label: 'Digital therapeutics', category: 'digital' },
  { slug: 'sleep_intervention', label: 'Sleep intervention', category: 'lifestyle' },
  { slug: 'nutrition', label: 'Nutrition/supplements', category: 'lifestyle' },
  { slug: 'exercise', label: 'Exercise plan', category: 'lifestyle' },
  { slug: 'lifestyle', label: 'Lifestyle intervention', category: 'lifestyle' },
  { slug: 'accommodations', label: 'School/workplace accommodations', category: 'environmental' },
  { slug: 'multimodal', label: 'Multimodal combined', category: 'combined' },
];
...
export function canUseInterventionAnalyzerWorkspace(role, opts = {}) {
  const normalized = String(role || '').trim().toLowerCase();
  if (!normalized) return !!opts.allowUnknown;
  return INTERVENTION_ANALYZER_CLINICAL_ROLES.has(normalized);
}
...
/** Neutral series-direction label for outcome sparkline / table — not "improving/worsening" as clinical fact */
function _trendArrow(trend) {
  const t = String(trend || '').toLowerCase();
  if (t === 'up' || t === 'improving') return '<span title="Scores moved in direction consistent with improvement on this scale (rule-based)" aria-label="Series direction up on scale" style="color:var(--text-secondary);font-weight:600">↑</span>';
  if (t === 'down' || t === 'worsening') return '<span title="Scores moved in direction consistent with worsening on this scale (rule-based)" aria-label="Series direction down on scale" style="color:var(--text-secondary);font-weight:600">↓</span>';
  if (t === 'flat' || t === 'stable') return '<span title="Little change between first and last plotted points" aria-label="Flat series direction" style="color:var(--text-tertiary);font-weight:600">→</span>';
...
function _errorCard(message, retryLabel = 'Try again') {
  const safe = esc(message || 'We couldn\'t load the intervention data right now.');
  return `<div role="alert" style="max-width:560px;margin:24px auto;padding:18px 20px;border:1px solid rgba(255,107,107,0.35);background:rgba(255,107,107,0.06);border-radius:12px">
    <div style="font-weight:600;margin-bottom:6px;color:var(--text-primary)">We couldn\'t load the intervention data right now.</div>
...
function _restrictedCard() {
  return `<div role="region" aria-label="Access restricted" style="max-width:560px;margin:48px auto;padding:24px;border:1px solid var(--border);border-radius:14px;background:var(--bg-card);text-align:center">
    ...
    <div style="font-size:12px;color:var(--text-secondary);line-height:1.6">
      Intervention analytics are restricted to authorised clinical staff roles. Follow your clinic\'s policy for patient-facing intervention summaries.
    </div>
  </div>`;
}
...
function _emptySessionsCard() {
  return `<div style="margin:18px 0;padding:18px 20px;border:1px dashed var(--border);border-radius:12px;background:rgba(255,255,255,.02);text-align:center">
    ...
    <div style="font-size:12px;color:var(--text-secondary);line-height:1.5">
      There are no delivered-session parameter rows for this course in the connected backend. This does not prove zero interventions occurred — verify documentation and device imports. Requires clinician review per clinic protocol if sessions should appear.
    </div>
  </div>`;
}
...
/**
 * Heuristic association-signal label based on session counts only.
 * NOT a validated clinical response classification. Requires clinician review.
 */
...
function _parseOutcomeSummaries(outcomesResp) {
  ...
  const ruleNote = 'Outcome series direction uses recorded outcome series for this course (backend rule-based deltas). Not a diagnosis or treatment-response label. Requires clinician interpretation.';
...
async function _hydrateCourseDetail(course, patientId, patientNameFromCaller = '', opts = {}) {
  ...
  const sessions = logs.map((s, i) => {
    ...
    return {
      ...
      intervention_type: _interventionTypeLabel(course.intervention_type_slug || clin?.intervention_type_slug || course.modality_slug || ''),
  ...
  return {
    course: {
      ...
      intervention_type: _interventionTypeLabel(course.intervention_type_slug || course.modality_slug || ''),
...
async function _loadClinicRowsFromApi(patientItemsForNames = null) {
  ...
  // BUG FIX 2: Parallelize hydration with concurrency limit (was serial N+1 fan-out)
  const rows = [];
  const courseIds = [];
  const courseEntries = [...byPatient.values()];
  const details = await _mapWithConcurrency(courseEntries, 5, async (c) => {
    ...
    return detail ? _buildClinicRow(detail) : null;
  });
  details.filter(Boolean).forEach((r) => rows.push(r));
  courseEntries.forEach((c) => courseIds.push(c.id));
...
function _renderClinicTable(rows, sortKey, sortDir) {
  ...
  return `<div style="background:var(--bg-card);border:1px solid var(--border);border-radius:14px;overflow:auto">
    <table style="width:100%;border-collapse:collapse;font-size:12px;min-width:820px" aria-label="Clinic intervention summary">
      <thead><tr>
        ...
        ${th('outcome_trend', 'Outcome series', 'center')}
...
function _renderCourseHeader(course, summary) {
  ...
  const interventionLabel = course.intervention_type || course.modality || '—';
  ...
  return `<div style="background:var(--bg-card);border:1px solid var(--border);border-radius:14px;padding:16px 18px;display:flex;gap:18px;align-items:center;flex-wrap:wrap">
    ...
    <div style="flex:1;min-width:240px">
      ...
      <div style="font-size:12px;color:var(--text-secondary);line-height:1.5">
        ${esc(interventionLabel)} · ${esc(course.target_site || '—')} · started ${esc(_fmtDate(course.started_at))}
      </div>
...
function _renderSparkline(scores, scaleName, ruleNote) {
  if (!Array.isArray(scores) || scores.length < 2) {
    return `<div style="margin-top:14px;padding:14px;background:var(--bg-card);border:1px solid var(--border);border-radius:12px">
      ...
      <div style="font-size:12px;color:var(--text-tertiary);line-height:1.5">${scores?.length === 1 ? 'Only one numeric point — series direction not plotted.' : 'Not enough outcome measurements linked to this course to plot.'} Outcome analytics may be unavailable or not configured.</div>
    </div>`;
  }
  const w = 480;
  const h = 80;
...
function _renderSessionRow(s, expanded) {
  ...
  const inline = expanded ? `<div style="margin-top:8px;padding:10px 12px;background:rgba(255,255,255,.02);border-radius:10px;font-size:12px;color:var(--text-secondary);line-height:1.5">
      ...
      <div><strong style="color:var(--text-primary)">Intervention type</strong>: ${esc(s.intervention_type || '—')}</div>
...
function _renderTimeline(sessions, expandedId) {
  ...
  return `<div style="margin-top:14px;background:var(--bg-card);border:1px solid var(--border);border-radius:12px;overflow:hidden">
    ...
    <div style="padding:8px 14px;font-size:11px;color:var(--text-tertiary);line-height:1.45">List completeness depends on sessions logged to this course in the backend — not proof of all interventions performed.</div>
...
function _buildClinicRow(detail) {
  ...
  return {
    ...
    course_id: course.id,
    course_label: `${course.intervention_type || course.modality || ''} · ${course.protocol_name || course.protocol_id || ''}`.trim() || '—',
...
export async function pgInterventionAnalyzer(setTopbar, navigate) {
  try { setTopbar('Intervention Analyzer', 'Multimodal intervention review · decision support only'); } catch {}
  ...
  el.innerHTML = `
    <div class="ds-intervention-analyzer-shell" style="max-width:1100px;margin:0 auto;padding:16px 20px 48px">
      <div id="ia-demo-banner"></div>
      <div style="padding:12px 14px;border-radius:12px;border:1px solid rgba(155,127,255,0.28);background:rgba(155,127,255,0.06);margin-bottom:14px;font-size:12px;line-height:1.45;color:var(--text-secondary)">
        ...
        Surfaces intervention course logs, outcome series, and sign-off state from connected records. Not autonomous treatment approval, protocol optimisation, dosing advice, or emergency triage. Follow clinic governance; interpret outcomes with source context.
      </div>
      <div id="ia-breadcrumb" style="display:flex;align-items:center;gap:10px;margin-bottom:12px;font-size:12px;flex-wrap:wrap"></div>
      <div id="ia-toolbar" style="margin-bottom:12px"></div>
      <div id="ia-body"></div>
    </div>`;

  if (!el.querySelector('[data-aar-strip="intervention"]')) {
    const _aarHost = document.createElement('div');
    _aarHost.dataset.aarStrip = 'intervention';
    el.prepend(_aarHost);
    mountAnalyzerAIReportStrip({
      container: _aarHost,
      analyzerType: 'intervention',
  ...
  function setBreadcrumb() {
    const bc = $('ia-breadcrumb');
    if (!bc) return;
    if (view === 'clinic') {
      bc.innerHTML = `<span style="font-weight:600">Interventions by patient</span>`;
    } else {
      bc.innerHTML = `<button type="button" class="btn btn-ghost btn-sm" id="ia-back" style="min-height:44px">← Back to clinic</button>
        <span style="color:var(--text-tertiary)">/</span>
        <span style="font-weight:600">${esc(activePatientName || 'Patient')}</span>`;
      $('ia-back')?.addEventListener('click', () => { view = 'clinic'; expandedSessionId = null; render(); });
    }
  }
  ...
  function _renderToolbarPatientSelect(patients) {
    ...
    tb.innerHTML = `
      <div style="display:flex;flex-wrap:wrap;gap:10px;align-items:center">
        <label for="ia-patient-select" style="font-size:12px;color:var(--text-secondary)">Select patient</label>
        <select id="ia-patient-select" class="btn btn-ghost btn-sm" style="min-height:44px;max-width:280px;border:1px solid var(--border);border-radius:8px;padding:8px 10px;background:var(--bg-card);color:var(--text-primary)">
          <option value="">— Choose patient —</option>
          ${opts}
        </select>
        <button type="button" class="btn btn-secondary btn-sm" id="ia-open-patient" style="min-height:44px" ${!activePatientId ? 'disabled' : ''}>Open profile</button>
        <button type="button" class="btn btn-ghost btn-sm" id="ia-refresh" style="min-height:44px">Refresh</button>
    ...
    tb.querySelector('#ia-patient-select')?.addEventListener('change', (ev) => {
    ...
    tb.querySelector('#ia-open-patient')?.addEventListener('click', () => {
    ...
    tb.querySelector('#ia-refresh')?.addEventListener('click', () => { render(); });
  }
  ...
  function _paintClinicTable(patients, loadingSign) {
    ...
    body.innerHTML = `
      ...
      <div style="margin-bottom:10px;display:flex;flex-wrap:wrap;gap:10px;align-items:center">
        <button type="button" class="btn btn-ghost btn-sm" id="ia-refresh-sign" style="min-height:40px">Refresh sign-off status</button>
    ...
    body.querySelector('#ia-refresh-sign')?.addEventListener('click', () => { loadClinic(); });
  ...
  async function loadClinic() {
    const body = $('ia-body');
    if (!body) return;
    const canUseWorkspace = canUseInterventionAnalyzerWorkspace(actorRole, { allowUnknown: isDemoSession() });
    if (!canUseWorkspace) {
  ...
  function wireClinicTable() {
    const body = $('ia-body');
    body?.querySelector('#ia-go-courses')?.addEventListener('click', () => {
      try { navigate?.('protocol-studio'); } catch {}
    });
    body?.querySelector('#ia-go-schedule')?.addEventListener('click', () => {
  ...
  function _renderOutcomePicker(outcomes) {
    ...
    return `<div style="margin-top:10px;display:flex;flex-wrap:wrap;align-items:center;gap:8px">
      <label for="ia-outcome-scale" style="font-size:12px;color:var(--text-secondary)">Outcome scale</label>
      <select id="ia-outcome-scale" class="btn btn-ghost btn-sm" style="min-height:40px;max-width:360px;border:1px solid var(--border);border-radius:8px;padding:6px 10px;background:var(--bg-card)">
        ${opts}
      </select>
  ...
  function _renderPatientDetail(detail, expandedId) {
    ...
    const canSignAll = canUseInterventionAnalyzerWorkspace(actorRole, { allowUnknown: usingFixtures && isDemoSession() });
    ...
    // Use provenance-aware response label if available
    const respLabel = course._response_label;
    const respNote = respLabel?.note || '';
    const respLabelHtml = respLabel?.label
      ? `<div style="margin-top:8px;font-size:11px;color:var(--text-tertiary)">Heuristic association signal: <strong>${esc(respLabel.label)}</strong> (${esc(respLabel.provenance || '')}) — ${esc(respNote)}</div>`
      : '';
    const governance = `
    ...
    return `${_safetyBanner()}${_renderCourseHeader(course, summary)}
    ${stale}
    ${respLabelHtml}
    ${_renderAeBanner(ae)}
    ...
    ${outcomes.responder_backend_flag != null ? `<div style="margin-top:8px;font-size:11px;color:var(--text-tertiary)">Backend association heuristic flag (rule-based, not clinical fact): ${outcomes.responder_backend_flag ? 'true' : 'false'} — requires clinician interpretation.</div>` : ''}
    ${governance}
  ...
  async function loadPatient() {
    ...
    // Attach provenance-aware response label
    if (detail.course) {
      detail.course._response_label = _responseLabel(detail.course);
    }
  ...
  function wirePatientDetail() {
    ...
    body.querySelector('#ia-outcome-scale')?.addEventListener('change', (ev) => {
    ...
    body.querySelector('[data-action="sign-all"]')?.addEventListener('click', async (ev) => {
      ...
      const canSignAll = canUseInterventionAnalyzerWorkspace(actorRole, { allowUnknown: usingFixtures && isDemoSession() });
      if (!canSignAll) {
        alert('Intervention sign-off is restricted to authorised clinical staff roles.');
        return;
      }
...
export default { pgInterventionAnalyzer };
