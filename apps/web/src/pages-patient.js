...
export async function pgPatientMarketplace(_user) {
  ...
  const FALLBACK_CATALOG = [
    ...
    { id: 'amz-book-cbt', kind: 'product', icon: '📖', tone: 'blue', name: 'Feeling Good: The New Mood Therapy', provider: 'David D. Burns', desc: 'The classic guide to cognitive behavioral therapy. Widely-used CBT techniques for depression and anxiety; this listing is the author’s own reference work and is not a DeepSynaps endorsement of any clinical claim.', price: '$10.39', priceUnit: '', clinical: false, featured: false, tags: ['Book', 'CBT', 'Depression', 'Anxiety'], external_url: 'https://www.amazon.com/dp/0380810336', seller: null },
...
function _wireSettingsPage() {
  ...
  // Session-revoke handler — honest no-op pending backend support.
  // The current "Active sessions" UI renders a disabled "—" placeholder
  // button with no `data-st-revoke` attribute, so this listener attaches
  // to zero nodes today. The handler is retained so that if a future
  // template re-adds a real revoke button, the click path can't ship a
  // success toast (or hide the row) before there is an API call backing
  // it. There is no `api.revokePatientSession` in apps/web/src/api.js
  // and no backend route to honour the click. See
  // ~/.hermes/audits/workflow-honesty-2026-05-11.md (P0 #1) for context.
  st.querySelectorAll('[data-st-revoke]').forEach(b => {
    b.addEventListener('click', () => {
      stToast('Session revocation is not yet supported in this portal. Contact your care team to revoke a session.');
    });
  });
