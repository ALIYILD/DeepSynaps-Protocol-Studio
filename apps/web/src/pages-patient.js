...
export async function pgPatientDashboard(user) {
  ...
  el.innerHTML = `
    ...
    <div class="card" style="margin-bottom:24px" onclick="window._navPatient('patient-wearables')" style="cursor:pointer">
      <div class="card-header" style="display:flex;justify-content:space-between;align-items:center">
        <h3>${t('patient.dash.wearable.title')}</h3>
        <button class="btn btn-ghost btn-sm" onclick="event.stopPropagation();window._navPatient('patient-wearables')">${t('patient.dash.wearable.view')}</button>
      </div>
      <div class="card-body" style="padding:14px 16px" id="pt-dash-wearable-preview">
        <div style="text-align:center;padding:12px 8px;color:var(--text-tertiary)">
          <div style="font-size:18px;margin-bottom:6px;opacity:.4">◌</div>
          <div style="font-size:12.5px;margin-bottom:10px">${t('patient.dash.wearable.empty')}</div>
          <button class="btn btn-ghost btn-sm" onclick="window._navPatient('patient-wearables')">${t('patient.dash.wearable.connect')}</button>
        </div>
      </div>
    </div>
  `;
...
export async function pgPatientWearables() {
  ...
  <div style="margin-bottom:24px">
    ...
    <div style="margin-top:14px">
      <div style="font-size:12px;font-weight:600;color:var(--text-secondary);margin-bottom:8px">◉ Notices from your care team</div>
      ${recentAlerts.map(flag => {
        const sevMap = {
          critical: { color: 'var(--red)',   bg: 'rgba(255,107,107,0.10)', label: 'Urgent' },
          warning:  { color: 'var(--amber)', bg: 'rgba(255,181,71,0.10)',  label: 'Heads up' },
          info:     { color: 'var(--blue)',  bg: 'rgba(74,158,255,0.10)',  label: 'Info' },
        };
        const sev = sevMap[flag.severity] || sevMap.info;
        return `<div style="display:flex;align-items:center;gap:10px;padding:9px 12px;border-radius:var(--radius-md);background:${sev.bg};border:1px solid ${sev.color}33;margin-bottom:6px">
          <span style="font-size:10px;font-weight:700;color:${sev.color};background:${sev.bg};padding:2px 7px;border-radius:99px;border:1px solid ${sev.color}55;flex-shrink:0">${sev.label}</span>
          <span style="font-size:12.5px;color:var(--text-primary);flex:1">${esc(flag.detail) || 'Your care team has flagged a recent health change.'}</span>
        </div>`;
      }).join('')}
    </div>` : ''}
  </div>
  ...
  <div style="margin-bottom:24px">
    ...
    ${!hasSummaryData
      ...
      : `<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:12px">
          ...
          ${latestPain    != null ? trendCard('Pain',    latestPain,    '/10', painVals,    'var(--red)',    'Wellness check-in', 'var(--red)')    : ''}
          ${latestAnxiety != null ? trendCard('Anxiety', latestAnxiety, '/10', anxietyVals, 'var(--violet)', 'Wellness check-in', 'var(--violet)') : ''}
        </div>
  ...
  function buildContext() {
    ...
    if (latestPain != null)    lines.push(`Pain level: ${latestPain}/10 (7-day: ${painVals.join(', ')})`);
    if (latestAnxiety != null) lines.push(`Anxiety level: ${latestAnxiety}/10 (7-day: ${anxietyVals.join(', ')})`);
    return lines.join('\n');
  }
