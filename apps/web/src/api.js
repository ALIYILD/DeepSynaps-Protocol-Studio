...
export const api = {
  ...
  // ── Patient Portal V2 (dashboard, wellness, home tasks, reports) ──────────
  getPatientDashboard: (patientId) => apiFetch(`/api/v1/patient-portal/dashboard?patient_id=${encodeURIComponent(patientId)}`),
  getPatientHomeTasks: (patientId, status = 'all') => apiFetch(`/api/v1/patient-portal/home-tasks?patient_id=${encodeURIComponent(patientId)}&status=${status}`),
  completeHomeTask: (taskId, patientId, data = {}) => apiFetch(`/api/v1/patient-portal/home-tasks/${encodeURIComponent(taskId)}/complete?patient_id=${encodeURIComponent(patientId)}`, { method: 'POST', body: JSON.stringify(data) }),
  submitWellnessCheckin: (patientId, checkin) => apiFetch(`/api/v1/patient-portal/wellness-checkin?patient_id=${encodeURIComponent(patientId)}`, { method: 'POST', body: JSON.stringify(checkin) }),
  getWellnessHistory: (patientId, days = 30) => apiFetch(`/api/v1/patient-portal/wellness-checkin/history?patient_id=${encodeURIComponent(patientId)}&days=${days}`),
  getSharedReports: (patientId) => apiFetch(`/api/v1/patient-portal/shared-reports?patient_id=${encodeURIComponent(patientId)}`),
  viewSharedReport: (reportId, patientId) => apiFetch(`/api/v1/patient-portal/shared-reports/${encodeURIComponent(reportId)}?patient_id=${encodeURIComponent(patientId)}`),
  getPatientEducation: (patientId, category = 'all') => apiFetch(`/api/v1/patient-portal/education?patient_id=${encodeURIComponent(patientId)}&category=${category}`),
  getUploadRequests: (patientId) => apiFetch(`/api/v1/patient-portal/uploads/requests?patient_id=${encodeURIComponent(patientId)}`),
  uploadPatientFile: (patientId, formData) => apiFetch(`/api/v1/patient-portal/uploads?patient_id=${encodeURIComponent(patientId)}`, { method: 'POST', body: formData }),
