...
export async function pgProtocolStudio(setTopbar, navigate) {
  ...
  window._studioExport = async () => {
    ...
    if (!S.patientId || !S.condition) {
      try { window._showToast?.('Select a patient and condition before exporting.', 'warning'); } catch {}
      return;
    }
    try {
      if (typeof api?.saveProtocol === 'function') {
    ...
    } catch (e) { try { window._showToast?.('Export failed: ' + (e?.message || 'endpoint error') + '. Draft preserved locally.', 'error'); } catch {} }
  };
