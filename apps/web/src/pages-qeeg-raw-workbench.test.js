...
test('workbench shell renders root container with clinical class', () => {
...
test('clinical CSS uses paper-tone background and indigo selected row', () => {
  const html = root.innerHTML;
...
test('paper-tone redesign exposes patient chip, clock, and AI-watching pulse', () => {
  const html = root.innerHTML;
...
test('top toolbar exposes all required controls', () => {
  const html = root.innerHTML;
...
test('top toolbar exposes both back-navigation buttons', () => {
  const html = root.innerHTML;
...
test('top toolbar exposes next-step buttons', () => {
  const html = root.innerHTML;
...
test('channel rail renders the 19-channel 10-20 montage (no ECG)', () => {
  const html = root.innerHTML;
...
test('right panel exposes all seven tabs and collapse toggle', () => {
  const html = root.innerHTML;
...
test('right panel tabs each carry a testid + the new is-active class hook', () => {
  const html = root.innerHTML;
...
test('AI Review tab declares the threshold-slider testid in source', () => {
...
test('ICA panel declares the 12-cell grid testid + Brain/Eye/Muscle/Mixed badges', () => {
...
test('decision-support disclaimer lives in the status bar tooltip, not the panel', () => {
  const html = root.innerHTML;
...
test('Best-Practice tab includes the cleaning quality checklist', () => {
  for (const item of [
...
test('Best-Practice tab exposes a clearer clinical next-step summary', () => {
  for (const item of [
    'Clinical next step',
    'Recommended now',
    'Readiness score',
...
test('Audit tab declares the audit-log testid', () => {
  assert.ok(WORKBENCH_SRC.includes('data-testid="qwb-audit-log"'), 'audit-log testid');
});

test('cleaning tools panel renders all four sections', () => {
...
test('Manual Analysis tab source includes signal quality, findings builder, and reference-only labels', () => {
  for (const item of [
...
test('AI Assistant panel shows safety wording in source', async () => {
...
test('Examples panel covers all canonical artefact archetypes', async () => {
  for (const ex of [
...
test('Audit panel labels are present in source', async () => {
  assert.ok(WORKBENCH_SRC.includes('Cleaning Audit Trail'), 'audit heading');
});

test('Best-Practice panel covers required topics', async () => {
...
test('Learning EEG reference is integrated without mirroring full site content', async () => {
...
test('canvas wrapper present for trace rendering', () => {
  const html = root.innerHTML;
  assert.ok(html.includes('id="qwb-canvas"'), 'canvas element');
  assert.ok(html.includes('id="qwb-canvas-wrap"'), 'canvas wrapper');
});

test('bottom status bar renders all required fields including amp + dirty marker', () => {
  const html = root.innerHTML;
...
test('immutable raw EEG notice is in the canvas overlay', () => {
  const html = root.innerHTML;
...
test('keyboard shortcuts modal is wired with new shortcuts', async () => {
  const html = root.innerHTML;
...
test('unsaved-edits modal is in the DOM with clinical wording', () => {
  const html = root.innerHTML;
...
test('exported entrypoint and helpers match router registration', () => {
...
test('source registers beforeunload guard for unsaved edits', async () => {
...
test('source wires Cmd/Ctrl+S to saveCleaningVersion', async () => {
  assert.ok(/(metaKey|ctrlKey)[\s\S]+saveCleaningVersion/.test(WORKBENCH_SRC), 'Cmd/Ctrl+S handler binds to save');
});

test('source wires Esc key to navBack', async () => {
  assert.ok(/'Escape'[\s\S]+navBack/.test(WORKBENCH_SRC), 'Esc handler routes to navBack');
});

test('source has post-rerun confirmation copy mentioning preserved raw EEG', async () => {
...
test('navBack opens unsaved modal when state.isDirty is true', () => {
...
test('navBack is a no-op-on-dirty path that does not call window._nav', () => {
  let navCalled = false;
...
test('workbench source references every backend API method', () => {
  const REQUIRED_API = [
...
test('capabilities panel selectors exist in source (stable UI surface)', () => {
...
test('every interactive button is wired to a handler', () => {
...
test('play button is wired to togglePlay (not a placeholder)', () => {
...
test('ICA reject/restore is wired through toggleICAComponent → postAnnotation', () => {
...
test('export bundle calls server-side summary in non-demo mode', () => {
...
test('title-bar menus dispatch to real actions, not placeholders', () => {
...
test('recording summary strip renders under the toolbar with key:value pairs and status pill', () => {
  const html = root.innerHTML;
...
test('live cursor readout exists in the status bar', () => {
  const html = root.innerHTML;
...
test('toolbar quick action row exposes Snapshot / Export / Save / Reprocess / Spectral buttons', () => {
  const html = root.innerHTML;
  for (const tid of [
    'qwb-quick-snapshot',
    'qwb-quick-export',
...
test('vertical tool selector is rendered at the left edge of the trace area', () => {
  const html = root.innerHTML;
...
test('every tool selector button carries non-empty title + aria-label', () => {
  const html = root.innerHTML;
  for (const tid of [
    'qwb-tool-select',
    'qwb-tool-bad-segment',
...
test('recording-strip status pill is "Untouched" when demo state has no cleaningVersion and no clinician edits', () => {
  const html = root.innerHTML;
...
test('recordingStatus rule is documented in source and keys strictly on isDirty', () => {
...
test('Recording Info card is wired into the Cleaning tab', () => {
  const body = document.getElementById('qwb-right-body');
  const html = (body && body.innerHTML) || '';
...
test('Band Power module is declared in source and renders inside the Best-Practice tab', () => {
...
test('Mini head map (10-20) is wired into the Cleaning tab', () => {
  const body = document.getElementById('qwb-right-body');
  const html = (body && body.innerHTML) || '';
...
test('Better window/event navigation: ◀ Prev / Next ▶ + event-prev / event-next', () => {
  const html = root.innerHTML;
...
test('seeded state.events render inline on the trace with paper-tone labels', () => {
  const html = root.innerHTML;
