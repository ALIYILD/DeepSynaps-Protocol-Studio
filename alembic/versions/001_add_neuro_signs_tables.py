"""
Alembic migration for NeuroSign tables (optional; use if managing migrations with Alembic).

If your project uses Alembic, generate a new migration:
    alembic revision --autogenerate -m "add neuro_signs tables"

Then manually apply or let Alembic handle it:
...
def upgrade():
    ...
    op.create_unique_constraint('uq_case_sign_clinician', 'case_neuro_signs', ['case_id', 'neuro_sign_id', 'clinician_id'])

    # Add check constraint for confidence
    op.create_check_constraint(
        'ck_case_neuro_signs_confidence',
        'case_neuro_signs',
    ...
    op.create_check_constraint(
        'ck_annotation_shape_type',
        'neuro_sign_annotations',
        "shape_type IN ('polygon', 'rectangle', 'ellipse', 'arrow', 'point')"
    )

    # Add foreign key constraint for annotation_id in case_neuro_signs
    op.create_foreign_key(
        'fk_case_neuro_signs_annotation_id',
        'case_neuro_signs',
        'neuro_sign_annotations',
        ['annotation_id'],
        ['id']
...
def downgrade():
    """Drop NeuroSign tables."""
    op.drop_constraint('fk_case_neuro_signs_annotation_id', 'case_neuro_signs', type_='foreignkey')
    op.drop_constraint('ck_annotation_shape_type', 'neuro_sign_annotations', type_='check')
    op.drop_table('neuro_sign_annotations')
    op.drop_constraint('ck_case_neuro_signs_confidence', 'case_neuro_signs', type_='check')
