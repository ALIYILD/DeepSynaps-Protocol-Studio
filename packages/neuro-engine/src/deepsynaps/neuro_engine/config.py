...
if BaseSettings is not None:
    ...
    class NeuroEngineSettings(BaseSettings):
        ...
        fastsurfer_output_root: Path = Field(default_factory=lambda: _default_root() / "outputs" / "fastsurfer")
    ...
    class NeuroEngineSettings:
        ...
        def from_env(cls, **overrides: Any) -> "NeuroEngineSettings":
            mapping: Dict[str, tuple[str, Any]] = {
                ...
                "default_participant_label": ("DEEPSYNAPS_NEURO_ENGINE_DEFAULT_PARTICIPANT_LABEL", "01"),
                "fastsurfer_subject_prefix": ("DEEPSYNAPS_NEURO_ENGINE_FASTSURFER_SUBJECT_PREFIX", "subject"),
                "fs_license_file": ("DEEPSYNAPS_NEURO_ENGINE_FS_LICENSE_FILE", None),
                "freesurfer_license_file": ("DEEPSYNAPS_NEURO_ENGINE_FREESURFER_LICENSE_FILE", None),
                "fmriprep_executable": ("DEEPSYNAPS_NEURO_ENGINE_FMRIPREP_EXECUTABLE", "fmriprep"),
                "fmriprep_docker_image": ("DEEPSYNAPS_NEURO_ENGINE_FMRIPREP_DOCKER_IMAGE", "nipreps/fmriprep:latest"),
                "fastsurfer_executable": ("DEEPSYNAPS_NEURO_ENGINE_FASTSURFER_EXECUTABLE", "run_fastsurfer.sh"),
                "fastsurfer_docker_image": ("DEEPSYNAPS_NEURO_ENGINE_FASTSURFER_DOCKER_IMAGE", "deepmi/fastsurfer:latest"),
                "segmentation_model_name": ("DEEPSYNAPS_NEURO_ENGINE_SEGMENTATION_MODEL_NAME", "dynunet"),
                "segmentation_weights": ("DEEPSYNAPS_NEURO_ENGINE_SEGMENTATION_WEIGHTS", None),
            }
            values: Dict[str, Any] = {}
