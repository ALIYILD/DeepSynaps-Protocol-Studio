...
Pydantic, OpenCV, FFmpeg bindings, or model runtimes. Higher-level analyzer
report contracts live in this module as Pydantic models so the scaffolded video
pipeline and the lean ingestion layer share one schema surface.
"""
...
# ---------------------------------------------------------------------------
# Pydantic report contract for the scaffolded analyzer pipeline
# ---------------------------------------------------------------------------
...
class SuggestedScore(BaseModel):
    """0-4 task score suggestion for clinician review only."""
...
class VideoAnalysisReport(BaseModel):
    """Top-level JSON contract returned by the scaffolded Video Analyzer."""
