...
export async function pgHomePrograms(setTopbar, navigate) {
  ...
  // ── Read all tasks across all known patients ─────────────────────────────
  const _loadAllTasks = () => {
  ...
  // ── Sync a task to patient-facing storage ────────────────────────────────
  ...
  // ── Save task to clinician store + bridge ────────────────────────────────
  ...
  // ── Get completions for patient ──────────────────────────────────────────
  const _getCompletions = pid => _ls(_compKey(pid), {});

  const _completionRate = (pid, tasks) => {
    const comps = _getCompletions(pid);
    const ptTasks = tasks.filter(t => t.patientId === pid && t.status !== 'archived');
    if (!ptTasks.length) return null;
    const done = ptTasks.filter(t => comps[t.id] || t.status === 'completed').length;
    return Math.round((done / ptTasks.length) * 100);
  };
  ...
  const _migrateIfNeeded = () => {
    ...
    legacy.forEach(t => {
      ...
      byPid[pid] = byPid[pid] || [];
      byPid[pid].push({ ...t, patientId: pid });
    });
    Object.entries(byPid).forEach(([pid, tasks]) => {
      const existing = _ls(_clinKey(pid), []);
      if (!existing.length) {
        _lsSet(_clinKey(pid), tasks);
        _registerPid(pid);
        tasks.forEach(t => _bridgeToPatient(pid, t));
      }
    });
  };
  _migrateIfNeeded();
  ...
  const _stats = () => {
    ...
    const rate = totalActive ? Math.round((totalComp / totalActive) * 100) : 0;
    const needFollowUp = new Set(tasks.filter(t => _isOverdue(t.dueDate)).map(t => t.patientId)).size;
    return {
  ...
  const _filtered = () => {
    ...
    if (_filter.search) {
      ...
      list = list.filter(t => {
        ...
        return (t.title || '').toLowerCase().includes(q) || name.includes(q) || (t.type || '').includes(q);
      });
    }
    return list;
  };
  ...
  const _ptName = pid => {
    ...
    if (pt) return [pt.first_name, pt.last_name].filter(Boolean).join(' ') || pt.name || pid;
    return pid;
  };
    ...
    return `
      <div class="hp-task-row ${isDone ? 'hp-task-done' : ''}" data-tid="${task.id}">
        ...
        <div class="hp-task-main">
          <div class="hp-task-title">${task.title}</div>
  ...
  const _adherenceView = () => {
    ...
    return `<div class="hp-adh-grid">${pids.map(pid => {
      const pt = patientMap[pid];
      const name = pt ? [pt.first_name, pt.last_name].filter(Boolean).join(' ') || pt.name : pid;
      ...
      const rateColor = rate >= 75 ? '#22c55e' : rate >= 40 ? '#f59e0b' : '#ef4444';
      return `<div class="hp-adh-card">
        <div class="hp-adh-name" onclick="window.openPatient('${pid}');window._nav('patient-profile')">${name}</div>
        <div class="hp-adh-bar-wrap"><div class="hp-adh-bar" style="width:${rate}%;background:${rateColor}"></div></div>
  ...
  const _modalHtml = (task, prefillPid) => {
    ...
    const patOpts = (apiPatients || []).map(p => {
      const name = [p.first_name, p.last_name].filter(Boolean).join(' ') || p.name || p.id;
      return `<option value="${p.id}" ${p.id === pid ? 'selected' : ''}>${name}</option>`;
    }).join('');
    const courseOpts = pid
      ? (coursesByPatient[pid] || []).map(c => `<option value="${c.id}" ${task?.courseId === c.id ? 'selected' : ''}>${c.condition || c.protocol_name || c.id}</option>`).join('')
      : '';
    const typeOpts = TASK_TYPES.map(t => `<option value="${t.id}" ${task?.type === t.id ? 'selected' : ''}>${t.icon} ${t.label}</option>`).join('');
      ...
      <div class="hp-modal-overlay" onclick="window._hpCloseModal()">
        <div class="hp-modal" onclick="event.stopPropagation()">
          <div class="hp-modal-header">
            <span>${task ? 'Edit Task' : 'Assign Home Task'}</span>
          ...
          <div class="hp-modal-body">
            ...
            ${task ? `<input type="hidden" id="hp-m-taskid" value="${task.id}">` : ''}
          </div>
          <div class="hp-modal-footer">
            <button class="hp-act-btn" onclick="window._hpCloseModal()">Cancel</button>
            <button class="hp-act-btn hp-act-primary" onclick="window._hpSubmitTask()">${task ? 'Save Changes' : 'Assign Task'}</button>
          </div>
        </div>
      </div>`;
  };
  ...
  const renderPage = () => {
    ...
    const topActions = `
      <div class="hp-top-actions">
        ...
        <button class="hp-act-btn" onclick="window._hpViewTemplates()">📚 Templates</button>
        <button class="hp-act-btn ${_view==='adherence'?'hp-act-active':''}" onclick="window._hpToggleView('adherence')">📊 Adherence</button>
        <button class="hp-act-btn ${_view==='queue'?'hp-act-active':''}" onclick="window._hpToggleView('queue')">☰ Task Queue</button>
      </div>`;

    const filterBar = `
      <div class="hp-filter-bar">
        <input class="hp-search" type="text" placeholder="Search patient, task, type…" value="${_filter.search}" oninput="window._hpSearch(this.value)">
        <select class="hp-filter-sel" onchange="window._hpFilterPid(this.value)">
          ...
          ${(apiPatients||[]).map(p => {
            ...
            return `<option value="${p.id}" ${_filter.pid===p.id?'selected':''}>${n}</option>`;
          }).join('')}
        </select>
    ...
    if (_view === 'adherence') {
      ...
      mainContent = `<div class="hp-card"><div class="hp-card-title">Task Templates & Library</div><div class="hp-tpl-grid">${tpls.map(_tplCard).join('')}</div></div>`;
    } else {
      ...
      const doneTasks    = filtered.filter(t => t.status === 'completed').slice(0, 10);
    ...
    const modalHtml = _showModal ? _modalHtml(_editingTask, '') : '';
  ...
  window._hpOpenAssign = (prefillPid) => {
    ...
    renderPage();
    if (prefillPid) {
      const sel = document.getElementById('hp-m-pid');
      if (sel) { sel.value = prefillPid; }
    }
  };

  window._hpCloseModal = () => {
    _showModal = false;
    _editingTask = null;
    renderPage();
  };
  ...
  window._hpModalPatient = (pid) => {
    ...
    const courseEl = document.getElementById('hp-m-course');
    if (!courseEl) return;
    const courses = coursesByPatient[pid] || [];
    courseEl.innerHTML = '<option value="">None</option>' + courses.map(c =>
      `<option value="${c.id}">${c.condition || c.protocol_name || c.id}</option>`).join('');
  };
  ...
  window._hpSubmitTask = () => {
    ...
    if (!pid || !title) {
      window._showNotifToast?.({ title: 'Required', body: 'Patient and task title are required.', severity: 'warn' }); return;
    }
    const task = {
      id: existId || ('htask-' + Date.now()),
      patientId: pid, title, type, reason, courseId: course,
      dueDate: due, frequency: freq, instructions: instr,
      assignedBy: window._currentUser?.name || 'Clinician',
  ...
  window._hpToggleView = (v) => { _view = v; renderPage(); };
  window._hpViewTemplates = () => { _view = 'templates'; renderPage(); };
...
with open(target, 'a', encoding='utf-8') as f:
    f.write(code)
print('Done appending pgHomePrograms')
